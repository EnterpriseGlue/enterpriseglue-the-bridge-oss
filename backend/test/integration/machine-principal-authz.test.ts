...
let customProjectDeployerRoleId = '';
...
describe('machine-principal authorization parity (database)', () => {
  beforeAll(async () => {
    ...
    const customRole = await permissionService.createCustomRole({
      key: `custom.project.machine-deployer-${prefix}`,
      name: `${prefix} machine deployer`,
      scope: 'project',
      permissionIds: [ProjectPermissions.DEPLOY],
      createdById: userId,
    });
    customProjectDeployerRoleId = customRole.id;
    ...
    for (const principal of [
      ...
      await permissionService.assignRole({
        ...
        ...principal,
        roleId: customProjectDeployerRoleId,
        scopeType: 'project',
  ...
  afterAll(async () => {
    ...
    try {
      ...
      await dataSource.getRepository(RbacRolePermission).delete({ roleId: customProjectDeployerRoleId });
      await dataSource.getRepository(RbacRole).delete({ id: customProjectDeployerRoleId });
  ...
  it('gives API clients and service accounts the same bounded project decision through a custom role', async () => {
    if (skip) return;
    for (const principal of [
      { principalType: 'api_client' as const, principalId: apiClientId },
      { principalType: 'service_account' as const, principalId: serviceAccountId },
