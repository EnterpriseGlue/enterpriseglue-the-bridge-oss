...
describe('migration OpenAPI contracts', () => {
  it('documents only the implemented generate endpoint with the shared engine-compatible plan shape', () => {
    const document = generateOpenApi();
    const paths = document.paths;
    const schemas = document.components?.schemas;

    expect(paths?.['/mission-control-api/migration/generate']?.post).toBeDefined();
    expect(paths?.['/mission-control-api/migration/plan/generate']).toBeUndefined();
