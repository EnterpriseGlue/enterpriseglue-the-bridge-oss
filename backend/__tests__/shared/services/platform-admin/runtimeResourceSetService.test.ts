...
describe('runtimeResourceSetService', () => {
  it('uses the supplied transaction store for create, update, and archive lifecycle writes', async () => {
    ...
    const created = await runtimeResourceSetService.create({
      ...
      selector: { mode: 'prefix', prefix: 'payments-' }, source: 'config', sourceRef: 'config_bundle:access',
    }, store as any);
    await runtimeResourceSetService.update(created.id, {
      ...
      sourceHash: 'hash-2', lastAppliedAt: 200, driftStatus: 'in_sync', isArchived: false,
    }, store as any);
    ...
    expect(insert).toHaveBeenCalledWith(expect.objectContaining({
      tenantId: 'tenant-a', runtimeResourceSetKeyIdentity: 'tenant-a:payments', source: 'config',
    }));
    expect(update).toHaveBeenNthCalledWith(1, { id: created.id }, expect.objectContaining({
      ...
      runtimeTenantId: 'runtime-a', sourceHash: 'hash-2', isArchived: false,
    }));
