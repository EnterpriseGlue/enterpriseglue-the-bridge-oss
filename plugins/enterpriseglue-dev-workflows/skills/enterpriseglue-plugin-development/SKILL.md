...
1. Read `../../references/repository-lifecycle.json` and run the plugin-root
   lifecycle guard for the intended operation before selecting a target. A
   retired repository is historical-only and never a forward-development target.
2. Resolve the active repository and classify the change before editing:
   ...
   - proprietary product capability: the private repository that owns that
     plugin.
3. Keep the host generic. Expose a documented public contract when a plugin
   needs a new capability; do not import proprietary plugin code into OSS or
   add product-specific behavior to host modules.
4. Preserve compatibility across the public plugin API, host packages, plugin
   manifest and distribution lock, installer, manager, Helm charts, signatures,
   and runtime loading boundaries.
5. Test OSS platform changes with the repository's plugin-platform, package,
...
6. Keep public technical contracts and operator guidance in OSS. Route internal
...
7. Release OSS host/platform artifacts and each plugin independently. Never add
   an OSS-to-EE synchronization or release dependency.
8. Let owning plugin repositories consume reusable OSS contract workflows and
