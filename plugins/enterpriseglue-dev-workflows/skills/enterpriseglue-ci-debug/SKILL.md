...
1. Read `../../references/repository-lifecycle.json` before resolving a run.
   Exclude retired repositories unless the user explicitly requests a read-only
   historical audit; never rerun or repair their workflows.
2. Resolve the PR, head SHA, repository, and failing run. Read
   `.windsurf/workflows/ci-debug.md` when present.
3. Inspect the actual failing job, step, annotation, and log text using GitHub
   metadata or `gh`; do not diagnose from the workflow name alone.
4. If `release-notes-preflight / Release-note preflight` failed, diagnose it
...
5. Classify other outcomes as product regression, test defect/flakiness,
