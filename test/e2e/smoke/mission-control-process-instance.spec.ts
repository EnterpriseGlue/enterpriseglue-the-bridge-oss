...
async function expectProcessInstanceState(page: Page) {
  await expect(page).toHaveURL(new RegExp(`/mission-control/processes/instances/${instanceId}$`));
  await expect
    .poll(async () => {
...
test.describe('Smoke: Mission Control process instance detail', () => {
  ...
  test('process instance detail loads @smoke', async ({ page }) => {
    ...
    await expectProcessInstanceState(page);
  });
});
