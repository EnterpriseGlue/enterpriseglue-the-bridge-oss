...
const FIXTURE_NOW = 50_000_000;
const PLUGIN_ID = 'io.enterpriseglue.mysql-load';
...
const database = requiredEnvironment('MYSQL_DATABASE');
...
try {
  ...
  const enqueued = await Promise.all(
    Array.from({ length: EVENT_COUNT }, (_, index) =>
      stores[index % stores.length].enqueue({
        pluginId: PLUGIN_ID,
        deploymentRef: 'mysql-load-deployment',
        tenantRef: `mysql-load-tenant-${index % 16}`,
  ...
  if (new Set(enqueued.map((item) => item.deliveryId)).size !== EVENT_COUNT) {
    throw new Error(
      'MySQL load enqueue lost or duplicated a delivery identity',
    );
  }
  ...
  if (duplicate.deliveryId !== enqueued[0].deliveryId) {
    throw new Error(
      'MySQL load replay did not preserve event idempotency',
    );
  }
  ...
  const deliveredByWorker = await Promise.all(
    stores.map((store, index) =>
      drainStore(store, `mysql-load-worker-${index + 1}`),
    ),
  );
  ...
  if (deliveredByWorkers !== EVENT_COUNT) {
    throw new Error(
      `MySQL load workers delivered ${deliveredByWorkers}/${EVENT_COUNT} events`,
    );
  }
  if (totalElapsedMs > REGRESSION_CEILING_MS) {
    throw new Error(
      `MySQL synthetic load exceeded ${REGRESSION_CEILING_MS}ms: ${Math.round(
        totalElapsedMs,
      )}ms`,
    );
  }
  ...
  const [deliveryState] = await migrationSource.query(
    `SELECT
       ...
       SUM(CASE WHEN event_json <> '{}' THEN 1 ELSE 0 END) AS retained_payload_count,
       SUM(
         CASE
  ...
  if (
    ...
    throw new Error(
      `MySQL synthetic load invariants failed: ${JSON.stringify({
        deliveryState,
      })}`,
    );
  }
...
async function drainStore(store, workerRef) {
  ...
  while (true) {
    ...
    if (
      ...
      throw new Error(
        `MySQL load worker ${workerRef} did not persist delivery`,
      );
    }
...
function incidentEvent(index) {
  return {
    specversion: '1.0',
    id: `mysql-load-event-${index}`,
    source: 'enterpriseglue-oss',
    type: 'io.enterpriseglue.host.incident.v1',
    subject: `mysql-load-incident-${index}`,
    time: '2026-07-27T05:00:00.000Z',
    dataschema:
      'https://schemas.enterpriseglue.io/events/incident-v1.json',
    tenantRef: `mysql-load-tenant-${index % 16}`,
    data: {
      engineRef: 'engine-mysql-load',
      incidentRef: `mysql-load-incident-${index}`,
