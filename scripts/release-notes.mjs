...
const generatedReleaseDocumentHeader = `---
doc_class: technical
...
export function renderReleaseNotes(fragments, { version = 'next', baseRef = '' } = {}) {
  if (fragments.length === 0) {
    return `${generatedReleaseDocumentHeader}# EnterpriseGlue ${version === 'next' ? 'Release Notes Preview' : `v${version} Release Notes`}\n\nNo release-note fragments changed since \`${baseRef || 'the selected baseline'}\`.\n`
  }
  ...
  let output = generatedReleaseDocumentHeader
