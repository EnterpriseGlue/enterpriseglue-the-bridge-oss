...
if [[ ! -f "$backend_env_file" || ! -f "$compose_file" || ! -f "$backend_expose_file" ]] || ! is_local_url "$base_url" || ! is_local_url "$api_url"; then
...
docker compose --project-directory "$repo_root" --env-file "$backend_env_file" \
  -f "$compose_file" -f "$backend_expose_file" up --detach backend >/dev/null
for _ in {1..90}; do
...
. "$backend_env_file"
set +a
db_endpoint="$(docker compose --project-directory "$repo_root" --env-file "$backend_env_file" -f "$compose_file" -f "$backend_expose_file" port db 5432 | sed -n '1p')"
test -n "$db_endpoint"
...
pnpm exec playwright test test/e2e/operaton-backstop-browser.spec.ts --config test/e2e/playwright.config.ts
