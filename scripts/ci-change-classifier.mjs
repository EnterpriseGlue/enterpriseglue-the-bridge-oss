...
const classifiers = {
  ...
  native_tenancy: [
    ...
    /^packages\/frontend-host\/proxy-routes\.json$/,
    /^frontend\/vite\.config\.ts$/,
    /^infra\/cdn\/plugin-routing\//,
    /^packages\/frontend-host\/src\/(?:contexts\/AuthContext\.tsx|plugins\/nativePluginRuntime\.(?:tsx|test\.tsx)|utils\/(?:invitationRoute|httpInterceptor)\.ts)$/,
    /^frontend\/__tests__\/src\/utils\/httpInterceptor\.test\.ts$/,
    /^frontend\/__tests__\/src\/contexts\/AuthContext\.test\.tsx$/,
    /^infra\/docker\/compose\/docker-compose\.keycloak-tls\.yml$/,
    /^frontend\/nginx\.conf$/,
    /^infra\/docker\/keycloak\/local-tls-frontend\.nginx\.conf$/,
    /^packages\/shared\/src\/(?:services\/(?:AuthSessionService|invitations|platform-admin\/(?:IdentityProviderProvisioningService|ProjectMemberService|EngineService|project-member-role-assignments|legacy-project-role-assignments|permissions))|middleware\/auth|utils\/jwt|infrastructure\/persistence\/entities\/(?:RefreshToken|User|IdentityProvider|Invitation))\.ts$/,
    /^packages\/backend-host\/src\/modules\/(?:auth\/routes\/(?:refresh|logout|identity-oidc|onboarding|sso-state)|tenancy\/routes\/tenants|invitations\/routes\/invitations)\.ts$/,
    /^packages\/shared\/src\/schemas\/platform-admin\/invitation\.ts$/,
    /^(?:packages\/frontend-host\/src\/pages|frontend\/__tests__\/src\/pages)\/AcceptInvite\.(?:tsx|test\.tsx)$/,
    /^backend\/(?:__tests__|test)\/.*(?:authSessionLineage|sessionRevocationRace|pooledIdentityLinking|pooledInvitationEnrollment)\.test\.ts$/,
