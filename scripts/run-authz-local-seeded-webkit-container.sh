...
docker run --rm --init \
  ...
  -w /work \
  "$container_image" \
  bash -lc 'corepack pnpm install --frozen-lockfile --offline >/dev/null && case "$PLAYWRIGHT_CONTAINER_SUITE" in seeded-smoke) seed_dir="$(mktemp -d /tmp/enterpriseglue-authz-webkit.XXXXXX)"; trap "rm -rf \"$seed_dir\"" EXIT; E2E_SEED_FILE="$seed_dir/user.json" corepack pnpm exec playwright test test/e2e/smoke/login.spec.ts test/e2e/smoke/access-control-local.spec.ts test/e2e/smoke/fine-grained-access-local.spec.ts test/e2e/smoke/variable-access-control-local.spec.ts --config test/e2e/playwright.config.ts ;; accessibility) E2E_SEED_USER=false E2E_DIRECT_DB_CLEANUP=false corepack pnpm exec playwright test test/e2e/access-control-accessibility.spec.ts test/e2e/identity-administration-accessibility.spec.ts --config test/e2e/playwright.config.ts ;; esac'
