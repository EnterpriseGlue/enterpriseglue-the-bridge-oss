...
| EG_TENANT_RELEASE_EFFECT_COHORT_EPOCH | Managed-release settlement foundation | unset | Positive durable effect-admission epoch for the exact host release. Required when managed pooled Cloud configures a release identity; ordinary self-host installations may omit both. Incomplete source coverage keeps shutdown verification ineligible. |
...
| EG_PLATFORM_CLOUD_IDENTITY_AUDIENCE | Required to use the platform Cloud identity exchange | unset | Exact Cloud control-plane audience for short-lived platform tenant-list and tenant-lifecycle identity assertions. Keep it distinct from the workload receipt issuer and tenant identity audience. The host still starts and the exchange returns 503 while it is unset. |
| EG_CLOUD_ACCOUNT_IDENTITY_ENABLED | No | false | Enables platform-level OIDC/SAML account entry for managed pooled Cloud signup. Requires `EG_TENANCY_MODE=pooled` and `EG_TENANCY_CLOUD_REQUIRED=true`; it does not enable self-hosted account registration. |
...
| EG_MANAGED_ENGINE_INTERNAL_DNS_SUFFIX | Required for workload-managed internal engines | unset | Exact Kubernetes service suffix for host-owned managed Operaton engines, for example `managed.svc.cluster.local`. Only `http://egme-<40 lowercase hex>.<suffix>:8081/engine-rest` is accepted; the ordinary private-host and insecure-HTTP opt-ins and a matching engine allowlist are still required. |
