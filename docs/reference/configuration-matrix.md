...
| EG_PLATFORM_CLOUD_IDENTITY_AUDIENCE | Required for cloud-required mode | unset | Exact Cloud control-plane audience for short-lived platform tenant-list and tenant-lifecycle identity assertions. Keep it distinct from the workload receipt issuer and tenant identity audience. |
