...
| Live authorization enforcement | A second disposable Operaton fixture enables HTTP Basic authentication and native authorization checks. A member of the synchronized group can read the deployed process and decision definitions; a non-member cannot. After owned-grant rollback, the former member is denied again. The sidecar, not EnterpriseGlue, holds the fixture engine credential. | `test:operaton-sidecar-backstop-container` |
