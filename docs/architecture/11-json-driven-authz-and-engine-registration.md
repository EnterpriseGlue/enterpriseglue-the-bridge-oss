...
- [ ] ⬜ Keep shared types generated/imported from shared schemas instead of duplicating hand-maintained frontend interfaces. The Platform Admin hook now imports the canonical identity-provider lifecycle, mapping, Effective Access, runtime-resource, and configuration apply/run/reconciliation-task response types from shared schemas; its explicit `scope` entitlement union remains only to render pre-migration rows. Other legacy frontend interfaces still require consolidation.
