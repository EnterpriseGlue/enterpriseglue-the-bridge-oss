...
- [ ] ⬜ Stop project/engine/member/governance services from calling `syncLegacyRoleAssignments`; write canonical assignments at the originating command boundary. `EngineService` is complete: owner/delegate governance mutations update managed canonical assignments directly, and legacy `EngineMember` operator/deployer updates/removals directly upsert/remove their stable `source = legacy` canonical assignments without invoking the batch synchronizer. `ProjectMemberService` and both project-creation paths now write/remove the corresponding per-user project-role assignments directly, including the transactional Starbase creation flow. Only the separate governance compatibility path remains to be migrated before this item is complete.
