...
- [x] ✅ Extend the non-secret migration assistant to legacy SAML records. It creates a disabled direct-SAML draft with copied non-secret endpoints and an explicit signing-certificate reference placeholder; legacy certificate ciphertext is never copied, decrypted, or returned.
