...
- [x] ✅ No customer has a deployed legacy SSO provider or mapping as of 2026-07-20, so representative migration/cutover evidence is not a release gate for this clean-slate rollout. The deployed cutover runbook remains available if that premise changes. Login discovery and tenant SSO enforcement now ignore legacy provider rows and use provider-neutral OIDC, SAML, and LDAP entries only; legacy evaluator and compatibility-provider removal remains a separate implementation task.
