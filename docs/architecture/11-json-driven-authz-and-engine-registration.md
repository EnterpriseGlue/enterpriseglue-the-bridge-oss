...
- [ ] ⬜ Replace engine/project collection builders and frontend role helpers with evaluator-backed visible collections and action decisions. The authenticated capability snapshot now derives Mission Control visibility from `EngineService.getUserEngines`, which discovers engines through canonical assignments only, carries the resolved tenant id through every evaluator check and engine discovery call, and no longer accepts or passes `platformRole` into permission evaluation; the auth session issuer has the same principal-only input. Starbase deployment-history and latest-artifact queries are now routed through one shared, visible-engine-filtered query service, and project deployment lists preserve their compatibility metadata while stripping persisted raw engine responses; engine import checks/previews, project creation imports, the engine role endpoint, project-member requester/target permission checks, deployment-diagnostics platform checks, Git sync/lock/repository/deployment/connection checks, and Mission Control deployed edit-target project-file checks explicitly propagate request tenancy to canonical evaluation. Mission Control surfaces that depend on exact runtime-resource resolution are explicitly `runtime-enforced`: the backend filters or evaluates the resolved resource, while the frontend coverage guard remains strict for every snapshot-guardable action. Dashboard context and aggregate file statistics now derive visible projects exclusively from the tenant-aware effective-permissions snapshot, including exact and global direct project grants, so retained `ProjectMember` rows cannot independently expand dashboard visibility; the backend authorization guard rejects any raw `ProjectMember` lookup in those two routes. The unused legacy `ProjectMemberService.getUserProjects` and Starbase `listForUser` collection builders have been removed; Starbase and other project-list routes use `project.visibleCollection`, and `ProjectMemberService` no longer infers owner access from project metadata. Project Overview now derives deployment availability from the scoped `project.deploy.create` action/permission plus server-returned eligible targets, never from `ProjectMember.role`. Engine page action helpers are now structurally restricted to an engine id, and focused coverage proves legacy display roles cannot enable an action; remaining frontend role helpers still need migration.
  ...
  Current partial migration: Project Members deploy-grant eligibility is now reported by the same canonical `project:files:edit` decision enforced by the mutation, and the modal uses that response rather than `ProjectMember.role`. Engine Members now protects directly assigned canonical owner/delegate principals from member-row mutation even when a retained display row has an ordinary role; the historical owner/delegate label remains a restrictive fallback until representative deployed-provider cutover evidence proves every governance row has its canonical grant. Other frontend role helpers remain in scope for this unchecked item.
