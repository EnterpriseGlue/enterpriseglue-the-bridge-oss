...
- [ ] ⬜ Change session and middleware contracts to use authenticated principal and tenant identity with evaluator decisions. New access, refresh, and onboarding JWTs carry only `principalType = user` and `principalId`; middleware normalizes pre-existing tokens and rejects mismatched user principals before database access. New tokens no longer emit `platformRole`, while older tokens remain parseable and their legacy claim has no authorization effect. Authenticated profile, local-login, onboarding, and direct-LDAP login payloads now expose a consistent response-only `session` context (`principal.type`, `principal.id`, and request-derived `tenant.id`), documented in OpenAPI; the tenant scope is deliberately not embedded in reusable JWTs. Tenant-aware evaluator decisions and removal of persisted `User.platformRole` response/migration data remain outstanding until all protected route families have parity tests.
