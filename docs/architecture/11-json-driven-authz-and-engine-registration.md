...
- [ ] ⬜ Extend `packages/frontend-host/src/features/platform-admin/hooks/useAuthzApi.ts` and platform settings/admin API types for identity, runtime resource, config-bundle, receipt, and reconciliation contracts. Canonical runtime-resource/resource-set list, materialization, and reconciliation contracts now live in the shared hook module; the role-assignment form, Access Control inventory panel, and Identity Mappings access/provisioning flows consume them. The remaining identity, config-bundle, and receipt surfaces are still being consolidated.
