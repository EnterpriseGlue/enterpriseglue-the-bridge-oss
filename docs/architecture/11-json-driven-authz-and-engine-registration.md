...
- [x] ✅ Remove `SsoClaimsMapping`, `SsoAssignmentMapping`, `SsoGroupMapping`, and their coverage evaluator. There are no customer SSO deployments to migrate; direct identity-provider entitlement mappings are the only supported authorization-mapping path.
...
- [x] ✅ No customer has a deployed legacy SSO provider or mapping as of 2026-07-20, so representative migration/cutover evidence is not a release gate for this clean-slate rollout. Login discovery, tenant SSO enforcement, and Access Control use provider-neutral OIDC, SAML, and LDAP entries only; legacy login, configuration, mapping, migration/cutover controls, API routes, and OpenAPI contracts are removed. Schedulers and administrative diagnostics no longer invoke a legacy mapping evaluator; removal of the now-unreachable backing implementation remains tracked separately.
