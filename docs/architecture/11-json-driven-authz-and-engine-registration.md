...
The Project Detail page and Project Members modal now import the shared `RoleAssignment` and principal-type contracts directly; project-specific aliases only name the UI context and no longer narrow or duplicate the transport schema. The remaining interfaces in this unchecked item still need consolidation.
