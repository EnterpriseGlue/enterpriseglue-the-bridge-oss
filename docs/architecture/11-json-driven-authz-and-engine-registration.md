...
- [x] ✅ Run provider-neutral identity mappings during direct SAML login synchronization, inside the same user/identity/group/assignment transaction. Legacy SSO group and direct assignment synchronizers still run for their existing contracts; Microsoft and Google direct-login convergence remains pending because their legacy provider identifiers are not yet reconciled to `IdentityProvider` records.
