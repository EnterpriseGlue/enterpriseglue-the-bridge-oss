...
- [x] ✅ Add the first engine-access bridge from Identity Mappings: an administrator can grant an engine role to the mapping's internal group for one engine or an Engine Set through the standard role-assignment API. Runtime-resource scopes and in-flow managed-group creation remain part of the broader wizard milestone.
