...
- [x] ✅ Add a provider-neutral migration assistant for persisted legacy Microsoft, Google, and generic OIDC provider records. It produces a disabled direct-OIDC draft using only non-secret metadata, requires a new secret reference plus callback/mapping/cutover steps, audits draft generation, and never decrypts or exposes legacy ciphertext. Environment-only legacy provider settings and final compatibility-route retirement remain pending.
