...
The generic tenant invitation flow now likewise has one shared request, readiness, reveal-once response, token, and onboarding contract. Its backend validator/response serializer, OpenAPI operation, and `InviteMemberModal` all consume that contract, so SSO/email readiness and manual invite fields cannot silently drift.
