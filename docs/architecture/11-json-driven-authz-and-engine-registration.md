...
- [ ] ⬜ Replace engine/project collection builders and frontend role helpers with evaluator-backed visible collections and action decisions. The authenticated capability snapshot now derives Mission Control visibility from `EngineService.getUserEngines`, which discovers engines through canonical assignments only, carries the resolved tenant id through every evaluator check and engine discovery call, and no longer passes `platformRole` into permission evaluation. The unused legacy `ProjectMemberService.getUserProjects` collection has been removed; Starbase and other project-list routes use `project.visibleCollection`. Remaining frontend role helpers still need migration.
