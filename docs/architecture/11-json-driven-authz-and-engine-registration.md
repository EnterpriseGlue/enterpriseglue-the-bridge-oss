...
- [ ] ⬜ Implement containerized protocol-faithful mock OIDC, SAML, and LDAP services for browser end-to-end testing. Disposable Keycloak-backed OIDC and signed-SAML localhost browser rehearsals now exercise the real authorization-code redirect and SAML HTTP-POST callback; the SAML trust anchor is fetched only from local metadata and consumed through an opaque file reference. The opt-in OpenLDAP harness exercises production-client LDAPS bind/group semantics, but still needs a browser-level direct-LDAP login lane. The in-process contract harness also exercises OIDC discovery/PKCE/JWKS, SAML attributes, and LDAP bind/group semantics.
