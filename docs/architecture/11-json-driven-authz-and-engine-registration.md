...
- [ ] ⬜ Match exact immutable external ids by default; display-name and regex matching require preview warnings and explicit platform enablement. The manual mapping flow defaults to exact and warns before `contains` or `exists`; new or newly enabled broad mappings are rejected unless the default-off `ssoBroadEntitlementMappingsEnabled` Platform Setting is explicitly enabled. Regex support remains intentionally unavailable in the provider-neutral mapping contract.
