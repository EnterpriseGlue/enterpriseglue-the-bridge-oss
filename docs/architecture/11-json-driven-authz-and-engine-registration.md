...
- [ ] ⬜ Complete the identity-provider membership/assignment source migration. Provider-neutral memberships now persist explicit provider-and-mapping lineage and normalize mapping-only rows during reconciliation; legacy `sso` membership and assignment sources still require their coordinated migration.
...
- [x] ✅ Provider-bound legacy direct SSO assignments now use the same `legacy_sso:<provider>:mapping:<mapping>` lineage and canonical assignment key, promote mapping-only rows in place during reconciliation, and retain `sourceMappingId` as mapping ownership for deletion. Dynamic Engine Set/materialization source migration remains in the coordinated queue.
...
- [x] ✅ Complete provider-id-bound generic OIDC/SAML start/callback flows and direct LDAP mode. Provider selection is bound into callback state and sessions, with same-protocol mismatch, forged/expired state, and SAML replay failures closed. LDAP claims-only reconciliation and legacy Microsoft/Google migration remain pending.
