...
- [x] ✅ Add a non-mutating migration-readiness check for a provider-neutral target. It reports exact direct-OIDC, enabled-state, secret-reference availability, and active identity-mapping blockers without revealing secret values.
- [x] ✅ Add an audited guarded cutover for persisted legacy Microsoft, Google, and OIDC provider records. `POST /api/identity/providers/legacy-cutover` rechecks readiness within the provider update transaction and disables only the selected legacy database record. Environment-based legacy authentication remains deployment-owned and is deliberately rejected by this endpoint.
...
- [x] ✅ Mirror the resulting legacy platform-role decision from Microsoft, Google, and legacy SAML provisioning into the same source-managed administrator group on every login. A resolved user role removes only the stale legacy-derived administrator membership; existing legacy claim/default-role evaluation remains enabled until its explicit provider-neutral conversion and retirement gate is complete.
- [x] ✅ Add a safe legacy SSO group-mapping conversion path. `POST /api/authz/sso-group-mappings/:id/migrate-provider-neutral` converts only group/role mappings using `equals`, `contains`, or `exists` into an explicit provider-bound identity mapping, retains the source mapping for verification, and is idempotent. Wildcard, negated, regex, custom-claim, and email-domain mappings require deliberate redesign rather than lossy conversion.
...
- [ ] ⬜ Complete provider-neutral conversion coverage for remaining legacy SSO mapping shapes. Exact legacy email-domain rules now convert to a sanitized `attribute` entitlement (`email_domain:<domain>`), emitted consistently by OIDC, SAML, LDAP, and test adapters; raw email addresses are not used as an entitlement. Identity providers now have a bounded `authorizationAttributeKeys` allowlist, configurable in JSON bundles and the Identity Provider form. Only allowlisted scalar or multi-value attributes are retained in normalized identity snapshots and emitted as deterministic `attribute:<key>:<value>` entitlements. Exact custom-claim legacy platform-role mappings can therefore convert only after the selected provider allowlists that claim name, and retirement coverage recognizes both safe attribute forms. Extend the equivalent guarded conversion flow to legacy group and exact-engine assignment mappings before considering this item complete. The SSO Role Mappings replacement dialog blocks the remaining unsupported rules before mutation and states the redesign requirement. Keep negated, regex, wildcard, and broad-selector mappings blocked from automatic conversion and provide a deliberate redesign path using groups, Engine Sets, and normal assignments.
...
- [ ] ⬜ Change session and middleware contracts to use authenticated principal and tenant identity with evaluator decisions. New access, refresh, and onboarding JWTs now carry `principalType = user` and `principalId`; middleware normalizes pre-existing tokens and rejects mismatched user principals before database access. Provider-neutral session issuance and refresh derive the compatibility admin value from the canonical `platform-administrators` group, preventing group-derived SSO administration from being lost on refresh. Tenant identity and evaluator-backed effective decisions remain outstanding. Retain `platformRole` as display/migration data only until all protected route families have parity tests.
...
- [ ] ⬜ Update `EngineSelector.tsx`, Dashboard context/UI, engine API types, and shared engine stores to consume permission-derived visibility instead of legacy `myRole` allowlists. `EngineSelector.tsx` and engine inventory actions now consume the backend permission snapshot/action decisions; Dashboard and shared stores remain.
...
- [x] ✅ Add `test:identity-contract`, `test:identity-integration`, serialized `test:legacy-auth-integration`, and `test:config-bundles` scripts plus the `test:authz-refactor` root aggregate. The legacy flow suite deliberately serializes files because its Express router fixtures have process-global module state; these commands run isolated protocol, route, mapping, and bundle suites. Containerized protocol-faithful browser mocks remain a separate pending milestone.
...
- [ ] ⬜ Change local/SSO session contracts to carry principal and tenant identity; treat any legacy platform-role field as display/migration data only until removed. User-principal JWT fields are now emitted and backward-compatible middleware normalization is in place; tenant identity and removal of legacy role data remain pending.
...
- [x] ✅ Add disabled-by-default `EG_CONFIG_*` settings to every Docker/OpenShift environment example and the configuration reference/matrix. Shared validation, backend `.env.example`, startup file bootstrap, optional status-only secret-reference preflight, fail-closed behavior, and sanitized health status are complete; self-host and published-image integration documentation remains pending.
