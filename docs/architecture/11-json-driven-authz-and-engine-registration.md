...
- [x] ✅ Provider-bound legacy SSO group mappings now persist `legacy_sso:<provider>:mapping:<mapping>` membership lineage and adopt mapping-only rows during reconciliation; global legacy mappings and assignment sources remain in the coordinated migration queue.
