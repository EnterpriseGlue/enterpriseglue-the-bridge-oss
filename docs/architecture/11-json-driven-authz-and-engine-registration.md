...
- [ ] ⬜ Stop project/engine/member/governance services from calling `syncLegacyRoleAssignments`; write canonical assignments at the originating command boundary. Engine owner/delegate governance mutations now update their managed canonical assignments directly and no longer invoke the legacy synchronizer. Legacy `EngineMember` operator/deployer rows, project membership, creation, and governance compatibility paths remain to be migrated before this item is complete.
