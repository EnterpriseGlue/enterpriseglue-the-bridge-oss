...
- [x] ✅ Align the legacy Google compatibility flow with the shared SSO state and post-provisioning contract: tenant and return-path context now survive the OAuth start/callback round trip, and successful provisioning invokes the shared reconciliation hook. Provider-neutral Google migration remains pending.
...
- [x] ✅ Add a non-mutating migration-readiness check for a provider-neutral target. It reports exact direct-OIDC, enabled-state, secret-reference availability, and active identity-mapping blockers without revealing secret values or disabling any provider. Automatic legacy archival remains deferred because legacy provider records are platform-global while target provider definitions may be tenant-scoped.
...
- [x] ✅ Add transactional provisioning for an existing group at `POST /api/identity/mappings/provision-access`. It creates the provider-neutral mapping and the scoped group assignment in one database transaction, under mapping, group, and role-management action checks. The Identity Mappings create flow adopts it for existing groups and supports engine, Engine Set, exact runtime-resource, and Runtime Resource Set scopes. Transactional in-flow group creation remains pending; the UI's new-group path is still the documented best-effort fallback.
