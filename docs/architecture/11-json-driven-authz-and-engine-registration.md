...
- [ ] ⬜ Preserve accountable owner/delegate fields only as governance metadata and require explicit effective role assignments for access. Engine access evaluation and member-list display use effective assignments, and the `my-engines` contract exposes accountable contacts under a distinct `governance` object while retaining legacy top-level fields for compatibility. Project member routes now require canonical delegate-management permission rather than trusting a legacy owner membership row; project-engine access auto-approval now requires the requesting user to hold canonical project-settings and engine-edit permissions rather than matching accountable metadata. The Engine Members modal no longer accepts an owner field it does not display or authorize from. Project metadata and remaining UI/API contract cleanup remain.
