...
- [x] ✅ Add `POST /api/authz/legacy-mapping-retirement/disable` as the explicit retirement operation. It requires the exact `RETIRE_LEGACY_MAPPINGS` confirmation token, rechecks the fail-closed readiness gate, disables rather than deletes legacy platform/group/engine mapping rows, and records an audit entry with the existing per-mapping Active control as the rollback path.
