...
- [x] ✅ Resolve live process-instance detail and instance-scoped read/mutation routes to their process-definition key before runtime-resource evaluation. Resource-aware process-instance lists issue engine queries only for authorized definition keys. Historical lists, preview/count endpoints, batch/migration, and dashboard aggregation remain pending.
