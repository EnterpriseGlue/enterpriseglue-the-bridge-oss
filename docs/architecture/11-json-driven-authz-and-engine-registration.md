...
- [x] ✅ Implement the narrowly scoped `mirrored_engine_backstop` for active Camunda 7 and Operaton engines, direct or customer-sidecar connected, with encrypted write-only group mappings, hash-bound preview/apply/rollback/drift receipts, config-managed secret references, a guarded Mission Control workflow, and a bounded sidecar adapter.
...
- [x] ✅ `engineRuntimeAuthorizationMode` enforcement with `enterpriseglue_authoritative` as the default and the narrow, receipt-gated `mirrored_engine_backstop` available for direct or customer-sidecar Camunda 7 or Operaton protection.
