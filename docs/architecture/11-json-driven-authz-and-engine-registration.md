...
- [ ] ⬜ Extend `packages/frontend-host/src/features/platform-admin/hooks/useAuthzApi.ts` and platform settings/admin API types for identity, runtime resource, config-bundle, receipt, and reconciliation contracts. Canonical runtime-resource/resource-set list and materialization contracts now live in the shared hook module, and the role-assignment form consumes them; the remaining identity, config-bundle, receipt, and reconciliation surfaces are still being consolidated.
