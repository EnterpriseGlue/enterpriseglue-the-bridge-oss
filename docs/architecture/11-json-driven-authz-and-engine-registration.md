...
- [ ] ⬜ Change session and middleware contracts to use authenticated principal and tenant identity with evaluator decisions. New access, refresh, and onboarding JWTs carry only `principalType = user` and `principalId`; middleware normalizes pre-existing tokens and rejects mismatched user principals before database access. New tokens no longer emit `platformRole`, while older tokens remain parseable and their legacy claim has no authorization effect. Tenant identity and evaluator-backed effective decisions remain outstanding. Retain the persisted `User.platformRole` response/migration data only until all protected route families have parity tests.
