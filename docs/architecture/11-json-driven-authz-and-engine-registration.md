...
- [x] ✅ Consolidate Engine Members UI contracts on the shared `EngineMembersResponse`, `EngineMember`, and `PendingEngineInvite` schemas. The modal no longer retains its divergent `grantedAt` shape, and custom effective roles render as neutral display labels rather than being treated as legacy member-role variants.
