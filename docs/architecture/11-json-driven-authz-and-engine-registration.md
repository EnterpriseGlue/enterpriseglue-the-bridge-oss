...
- [x] ✅ Complete the end-to-end alignment gate: canonical principal assignments, external identity links, provider-id-bound provider-neutral login, secure secrets, config ownership fields, project-target ownership, runtime assignment semantics, and legacy authorization removal are implemented and covered by the maintained local aggregate and authenticated localhost smoke.
...
- [x] ✅ Implement containerized protocol-faithful mock OIDC, SAML, and LDAP services for browser end-to-end testing. Disposable Keycloak-backed OIDC and signed-SAML localhost browser rehearsals exercise the real authorization-code redirect and SAML HTTP-POST callback; the SAML trust anchor is fetched only from local metadata and consumed through an opaque file reference. The opt-in OpenLDAP browser rehearsal starts a disposable LDAPS directory, writes its generated CA and bind password only into the ignored local file-reference mount, verifies directory connectivity, and signs in through the public direct-LDAP form. The in-process contract harness also exercises OIDC discovery/PKCE/JWKS, SAML attributes, and LDAP bind/group semantics.
...
- [x] ✅ Implement the narrowly scoped `mirrored_engine_backstop` for active, direct Camunda 7 and Operaton engines, with encrypted write-only group mappings, hash-bound preview/apply/rollback/drift receipts, config-managed secret references, and a guarded Mission Control workflow.
...
- [x] ✅ Make `enterpriseglue_authoritative` the default runtime authorization mode and keep `engine_native_authority` deferred. The separate narrow Camunda 7/Operaton mirrored backstop remains EnterpriseGlue-authoritative and is available only after a successful retained receipt.
...
- [x] ✅ `engineRuntimeAuthorizationMode` enforcement with `enterpriseglue_authoritative` as the default and the narrow, receipt-gated `mirrored_engine_backstop` available only for direct Camunda 7 or Operaton protection.
...
| --- | --- | --- |
| P0 | `role_assignments` uses the non-null canonical assignment key; migrations `0093`–`0095` remove historical user/resource/source-mapping columns and indexes. | Complete: canonical principal, scope, role, source, and source-ref identity is the sole evaluator/persistence contract. |
| P0 | Canonical assignments and policies authorize access; accountable owner/delegate and retained member rows are display/governance compatibility data. One migration-only local projection seeds existing local rows exactly once. | Complete: no runtime evaluator grant or repeated startup synchronization reads the compatibility rows. |
| P0 | Provider-neutral OIDC, SAML, and direct LDAP bind one selected persisted provider through login and reconciliation. | Keep all external sign-in on the provider-neutral paths; no legacy-provider cutover or migration path is supported. |
- [x] ✅ Retire legacy Microsoft/Google/SAML provider migration, readiness, cutover, callback, environment-fallback, and post-provisioning-hook paths. No customer has deployed SSO, so the supported surface is provider-neutral OIDC, SAML, and direct LDAP only.
...
| `mirrored_engine_backstop` | EnterpriseGlue remains the source of truth, but an exact, representable subset of group runtime `READ` access is mirrored to a direct Camunda 7 or Operaton endpoint. | Admins still edit EnterpriseGlue roles/groups/resource sets. Native authorization is shown only as sanitized mapping/sync/drift status, never as a second editor. | Implemented for active direct Camunda 7 and Operaton engines, including secret-reference configuration-bundle mappings and the guarded Mission Control workflow. Direct-identity-provider certification remains a release-evidence gate. |
...
| `identity-providers.json` | Provider-neutral OIDC/SAML/LDAP persistence, config apply, direct-login adapters, secret resolution, and connection testing exist | Add provider-API reconciliation beyond LDAP. |
...
- [x] ✅ Execute domain writes through existing role/group/engine/assignment/mapping/target services or shared lower-level commands used by both UI and bundle apply. Bundle create, update, archive, and authoritative reconciliation now route through the corresponding transaction-aware domain command; focused lifecycle/config suites and backend typecheck verify the integration.
...
- [ ] ⬜ `baseUrl` should point to the customer sidecar or gateway, not a public unauthenticated engine endpoint. Validate this in the deployed environment with the [customer sidecar readiness runbook](../how-to/customer-sidecar-readiness-runbook.md).
- [ ] ⬜ Sidecar endpoints should be private and network-restricted. Prefer mTLS, API-key references, or OAuth client credentials for the EnterpriseGlue-to-sidecar hop when the customer endpoint supports them. The runbook records the required network and upstream-authentication evidence.
- [ ] ⬜ The downstream peer-to-peer token and its rotation lifecycle remain customer-owned and must never appear in config bundles, engine APIs, logs, audits, UI fields, or support diagnostics. The runbook makes the ownership and rollback boundary explicit; deployed customer evidence is still required.
- [x] ✅ EnterpriseGlue audit records the effective user, canonical authorized action, project when present, engine, and request lineage for every authorized outbound engine operation. Shared request context is populated by action/composite/runtime authorization middleware and the bounded client writes only stable IDs plus sanitized operation/result enums; endpoint URLs, request bodies, credentials, and customer downstream tokens are excluded. Focused middleware and sidecar-client regression coverage proves successful and rejected sidecar calls preserve this lineage without leaking upstream response content.
...
- [ ] ⬜ Add optional Kubernetes Secret, Docker secret, and Vault adapters later. Docker Compose/Swarm-style mounted secrets now have a bounded `docker://<secret-name>` adapter that resolves only direct entries under `/run/secrets` (or an explicit mount root); Kubernetes/OpenShift projected secrets remain supported through the existing bounded `file://` adapter. A Vault adapter remains deferred.
...
- [x] ✅ Audit secret reference identifiers and change events, but never resolved values; unredacted-audit permission does not reveal credentials. Config-apply audit entries retain only deduplicated opaque reference identifiers alongside the existing object-change summary and canonical hash; they omit the config payload, resolved values, and reference locations. The Audit UI's elevated toggle is limited to PII presentation and does not alter credential redaction.
- [x] ✅ Keep reveal-once semantics for any generated API client credentials. API-client and service-account create/rotate responses are the only paths that return a token; persisted rows retain a one-way secret hash, list responses omit both hash and token, and rotation invalidates the previous token. The maintained secret-boundary lane verifies service and HTTP behavior for both credential types.
...
- [x] ✅ Match exact immutable external ids by default; display-name and regex matching require preview warnings and explicit platform enablement. The manual mapping flow defaults to exact and warns before `contains` or `exists`; new or newly enabled broad mappings are rejected unless the default-off `ssoBroadEntitlementMappingsEnabled` Platform Setting is explicitly enabled. Regex support remains intentionally unavailable in the provider-neutral mapping contract. Service coverage proves broad mappings fail before persistence until the setting is enabled; UI coverage proves the default exact selector and the broad-match warning.
- [x] ✅ Persist external subject, entitlement, provider, mapping, sync-run, and last-seen lineage without exposing raw token or directory payloads to normal users. Normalized snapshots persist tenant/provider/subject/user lineage, allowlisted entitlement claims, provider status, and last-seen timestamps; provider-mapping memberships and sync-run/event diagnostics retain source lineage. Snapshot and diagnostics contracts redact token, assertion, credential, certificate, and sensitive raw-attribute material.
- [x] ✅ Create group memberships with provider-managed source lineage and remove only rows owned by the same provider and mapping during authoritative sync. Reconciliation writes `source = "identity_provider"` plus provider/mapping source references; its authoritative cleanup targets that exact source lineage and preserves manual, API, automation, and other-provider rows.
...
- [x] ✅ Represent “all authenticated users” as an `exists` mapping to a configured internal group.
- [x] ✅ Add the provider-neutral `authenticated` entitlement emitted only after the adapter validates a subject. It is available through API/OpenAPI/config-bundle mapping schemas and can be mapped with the normal exact/exists mapping evaluator to an internal default group.
- [x] ✅ Implement the dedicated SSO/Identity Engine Assignment wizard. Identity Mapping creation now guides administrators through provider entitlement, internal group plus optional engine/Engine Set/runtime-resource scope, and an atomic-create review; edit mode keeps the existing single-screen compatibility form. The existing transactional endpoint still creates the mapping, optional group, and normal scoped group assignment together.
...
- [x] ✅ Run provider-neutral identity mappings once during normalized-identity upsert, inside the same user/identity/group/assignment transaction. Upsert returns the provider-managed membership counts without a duplicate synchronization pass.
- [x] ✅ Ensure every provider-neutral OIDC/LDAP/SAML provisioning transaction idempotently creates the source-managed global `authenticated-users` membership. The seeded group carries the non-privileged platform-user assignment, provisioning diagnostics count a newly created baseline membership, and the transaction fails rather than leaving a provisioned SSO user without this group-backed baseline.
...
- [x] ✅ Retire legacy SSO mapping conversion, coverage, verification, and cutover controls. No customer has legacy mapping rows to preserve; all new external authorization uses direct provider-neutral identity entitlement mappings and normal group assignments.
- [x] ✅ Guard the remaining `source = legacy` authorization writers. The backend structural guard permits them only in membership reconciliation, the migration-only project-role helper, and the documented `EngineProjectAccess` project-engine bridge; normal commands, including provider-neutral mapping conversion, must write canonical `manual` or `sso` provenance instead.
- [x] ✅ Remove the retired `SsoProvider`, `SsoClaimsMapping`, `SsoAssignmentMapping`, and `SsoGroupMapping` persistence and API surfaces, plus their coverage evaluator. There are no customer SSO deployments to migrate; direct identity-provider entitlement mappings are the only supported authorization-mapping path. The irreversible schema migration drops the obsolete `sso_providers` table.
...
- [x] ✅ Store provider-neutral mapping, group membership, assignment, and materialization ids in Effective Access explanation lineage so the extra indirection remains understandable. Effective Access and By Principal render provider-neutral mapping details and correlate their audit entries.

### Current Authorization Migration Status And Next Steps

The platform now has a group-backed baseline for new and existing active local, invited, bootstrap, and provider-neutral OIDC/SAML/LDAP users. Legacy `User.platformRole` remains compatibility data, while canonical group memberships establish access. Existing local legacy administrators retain their separate `source = system` compatibility membership until explicit retirement.

Implement the remaining work in this order:

- [x] ✅ Configure default access with explicit provider-neutral mappings to `authenticated-users` or, with the existing high-risk safeguards, `platform-administrators`.
...
| `mirrored_engine_backstop` | Implemented narrow Camunda 7/Operaton subset. | EnterpriseGlue remains the source of truth and projects only exact mapped-group `READ` grants for resolved process/decision definitions. | A direct Camunda 7 or Operaton endpoint enforces that subset for meaningful native group identity; all other source shapes remain blocked/manual. | EnterpriseGlue editor plus guarded sanitized Mission Control sync/backstop diagnostics. No separate engine permission editor. |
...
- [x] ✅ Synchronize only the explicitly supported exact mapped-group Camunda 7/Operaton `READ` subset behind a hash-bound preview/apply/rollback/drift protocol. EnterpriseGlue remains the final product evaluator and never invokes the engine after an EnterpriseGlue denial.
...
- [x] ✅ Add provider-neutral identity, entitlement, mapping, synchronization, runtime-scope, deployment-receipt, lineage-quality, runtime-resource, and config-bundle Zod schemas under `packages/shared/src/schemas/platform-admin/`. `identity.ts`, `engine-management.ts`, `deployment-receipt.ts`, `config-bundle.ts`, and `authz.ts` expose the canonical contracts, with shared schema coverage for identity, runtime-resource, deployment, and config-bundle boundaries.
- [x] ✅ Extend `packages/shared/src/schemas/platform-admin/authz.ts` with identity mapping, external identity diagnostics, runtime resource, runtime resource set, and effective-access input/output schemas. Identity-mapping request/response/provisioning, audited external-identity unlink recovery, runtime-resource/resource-set, effective-access (including its runtime route validator), SSO synchronization, provider lifecycle, connection, membership replay/preview, and migration readiness/cutover contracts are now canonical.
...
- [x] ✅ Replace legacy role-assignment uniqueness with principal/scope/source uniqueness and remove the persisted `userId`, `resourceType`, `resourceId`, and `sourceMappingId` aliases. The non-null canonical assignment key is the sole uniqueness contract, and migrations `0093`–`0095` remove the historical columns and indexes.
...
- [x] ✅ Retire persisted role-assignment `resourceType`/`resourceId` aliases. Scope type/id are the sole database and evaluator fields; migration `0094` removes the obsolete columns and index while transitional API fields are derived from the canonical scope.
- [x] ✅ Retire the persisted role-assignment `userId` alias. Canonical `principalType`/`principalId` are the sole database and evaluator identity fields; migration `0095` removes the obsolete column and index while user-facing API display remains derived from canonical user principals.
- [x] ✅ Make tenant-owned role keys and managed-resource keys deterministic, and persist configuration source/hash/ownership metadata on roles, groups, providers, mappings, engines, Engine Sets, runtime-resource sets, assignments, and targets. The configuration apply/diff lanes cover the resulting identities and ownership transitions.
...
- [x] ✅ Update every configured database adapter/entity registry and run migration tests against the supported OSS database baseline. A maintained adapter-registry guard verifies that PostgreSQL, MySQL, SQL Server, Oracle, and Spanner all register the canonical identity, configuration, runtime, and deployment entities; migration bootstrap/safety tests pass; and the PostgreSQL schema migration suite passed against an isolated, temporary database on the local Docker PostgreSQL service. The test accepts `MIGRATION_TEST_POSTGRES_*` overrides so it cannot mutate the active application database.

### Shared Services

- [x] ✅ Add `IdentityProviderAdapter` implementations for OIDC, SAML, and LDAP, and use them from provider-neutral normalized identity, entitlement-mapping, provisioning, and diagnostics orchestration. Maintained adapter and identity contract suites cover all three protocols.
...
- [x] ✅ Complete provider-id-bound OIDC, SAML, and direct-LDAP login orchestration: OIDC uses provider-bound state/PKCE/nonce/discovery/JWKS, SAML uses signed RelayState/assertion verification, and LDAP uses LDAPS lookup, user bind, and normalized groups. No legacy Microsoft/Google migration is required for this clean-slate rollout.
- [x] ✅ Converge external identity entitlement mapping on group-first mappings. The UI provisioning flow can create a managed internal group and ordinary scoped group assignment atomically; platform-role and direct-engine legacy mapping services are retired.
...
- [x] ✅ Extend `packages/frontend-host/src/features/platform-admin/hooks/useAuthzApi.ts` and platform settings/admin API types for identity, runtime resource, config-bundle, receipt, and reconciliation contracts. Canonical runtime-resource/resource-set list, materialization, reconciliation (including optional runtime/deployment skip status), identity-provider lifecycle/diagnostic and shared list query, identity-mapping (including read-only legacy scope rows), project deployment-target, and config-bundle apply/run/reconciliation-task response contracts and history queries now live in the shared hook module; the role-assignment form, Access Control and Engine detail inventory panels, Identity Mappings access/provisioning flows (including canonical group, role, Engine Set, and provider lists), Identity Providers UI, Configuration Bundles UI, and project deployment-target UI consume them. Engine receipt, deployment-history, and on-demand sanitized deployment-lineage reads now use one typed Mission Control API module and the engine detail shows the lineage artifacts. The API-client-only direct-pipeline receipt mutation shares its create and response schemas with backend validation, OpenAPI, and the receipt service; no browser mutation surface is exposed for that pipeline endpoint.
- [x] ✅ Refactor `packages/frontend-host/src/features/platform-admin/pages/AccessControl.tsx` into focused tab components as new surfaces are added; the behavior may change without restructuring the overall Carbon admin navigation. Shared Carbon data-table primitives plus the Roles, Permissions, Policies, Role Assignments, Runtime Resources, SSO platform/group mappings, SSO mapping claims preview, SSO engine-assignment diagnostics, sync, snapshot, mapping composition, Groups, Engine Sets, Project Engine Targets, and External Registration panels now live in focused components. The By Principal/By Resource summary models and table presentation now live in `principalResourcePresentation.ts` and `principalResourceTablePresentation.tsx`, while compatibility exports plus page-owned queries, mutations, permission decisions, and modals remain in the parent. The stale unmounted role/permission matrix was removed rather than retained as a second, unreachable role editor.
...
- [x] ✅ Update Mission Control engine selectors, process/decision lists, process details, batches, migrations, and dashboard widgets for authorized subsets. Engine discovery is permission-derived, runtime routes filter or resolve inherited resources, and all collection surfaces explain authorization-scoped empty results. Decision definition and decision-history queries additionally use the canonical runtime-read snapshot gate before issuing a browser request, rendering an explicit unavailable state rather than a blank selection surface when access is absent.
- [x] ✅ Update `DeployDialog.tsx`, project deployment-target UI, deployment history, and Mission Control-Starbase bridge diagnostics for proxy/direct ingestion and lineage quality. DeployDialog and Starbase share one project-engine access/eligibility contract, the target modal consumes the canonical project-target contract, and Mission Control deployment history can open sanitized runtime artifact lineage for an individual record. The proxy-deployment request/result is one shared schema used for backend validation, OpenAPI, the typed Mission Control client, and DeployDialog; it retains flat legacy request fields while browser clients use the nested options form. The direct-pipeline receipt mutation is intentionally machine-only, with shared create/response schemas, API deployment eligibility, idempotency, audit, OpenAPI, operator procedure, and receipt/history presentation. Starbase↔Mission Control denial feedback preserves the backend-evaluated missing requirements and offers its tenant-safe Effective Access diagnostic link without reproducing the composite authorization decision in the browser.
...
- [x] ✅ Run an ephemeral in-process LDAP server or CI container with deterministic users, groups, nested memberships, immutable group ids, DNs, paging, and TLS configuration. The opt-in LDAPS container generates a one-day CA and random credentials, seeds active and disabled users plus nested renamed immutable-id groups, proves one-entry-page enumeration and strict provider-scoped TLS, and removes the directory after each run. Separate lifecycle/referral behaviors remain tracked below.
- [ ] ⬜ Support bind success/failure, user search, `memberOf`, group reverse search, nested groups, disabled/deleted users, renamed group with stable id, deleted group, paging, referral behavior, timeout, and unavailable directory. The LDAPS harness verifies service/user bind success, rejected user credentials (normalized without protocol details), missing/deleted-user rejection, configurable-filter exclusion of a disabled user from direct login and enumeration, reverse/nested groups, immutable group IDs through a renamed group CN, deleted-group removal, and paging; a returned referral now fails closed with a stable provider-unavailable diagnostic. Additional referral-topology and network-edge cases remain pending.
...
- [x] ✅ Keep the implementation swappable: the shared LDAP adapter contract runs against the in-memory provider-neutral fake, and `DirectLdapIdentityService` receives an injected `LdapClientFactory` in its unit contracts. The disposable OpenLDAP container is therefore limited to production-client protocol fidelity rather than defining the authorization adapter contract.
...
- [x] ✅ Group rename with stable immutable id keeps membership; reused display name with different id does not inherit access. LDAP reverse-group lookup normalizes configured immutable IDs (deduplicating renamed/alias DNs), and mapping evaluation matches those IDs rather than display names, so a reused name with a different ID cannot inherit the prior mapping.
- [x] ✅ Additive synchronization preserves previous provider-managed membership; authoritative synchronization removes stale rows owned by that mapping only. Stateful reconciliation coverage proves a non-match retains additive, manual, and other-provider rows while deleting only the stale row derived from the current provider-qualified mapping reference.
...
- [x] ✅ Cross-tenant provider, group, mapping, principal, engine, and runtime-resource references are rejected. Identity mappings resolve providers and groups in their exact tenant and mutation lookups carry that same boundary; role-assignment creation now tenant-scopes group principals, projects, engines, engine sets, and runtime resources (while deliberately allowing global resources), with regressions for foreign groups, engines, mapping references, and runtime-resource sets.
...
- [x] ✅ Successful reconciliation records sanitized run/events and invalidates only affected principal/resource snapshots. Login and scheduled LDAP receipts persist sanitized run/events with exact provider-managed membership additions/removals; the permission snapshot version is derived from the affected user's tenant-scoped membership timestamps and visible resources, so reconciliation changes refresh that principal's snapshot without a global authorization-cache sweep.
...
- [x] ✅ Reject missing provider/group/role/engine references, duplicate keys, wrong-scope permissions, plaintext secrets, and test fixture imports. Preview rejects unresolved in-bundle groups, roles, and engines plus invalid permission scopes; the persisted-state diff rejects missing providers; schemas reject duplicate object keys, raw secret fields, and test-only or duplicate import paths without echoing secret values.
- [x] ✅ Prove UI-created manual objects survive authoritative config apply unless ownership is explicitly transferred. Manual roles and groups conflict before an apply transaction begins; authoritative cleanup archives only this bundle's rows; manual project targets are preserved until a reviewed `transferOwnership` reason is supplied, at which point the audit receipt records the previous source and transfer reason.
...
- [x] ✅ Add backend protocol-integration suites under `backend/test/integration/identity/` and keep existing service unit tests focused. OIDC discovery/authorization/token/JWKS/userinfo/groups and SAML metadata/browser-post validation run in the dedicated `test:identity-protocol-integration` lane; the optional LDAP container test remains separate because Docker is not a default test dependency.
...
- [x] ✅ Add an opt-in local LDAP mock Compose harness because LDAP protocol fidelity requires a directory container; OIDC/SAML continue to use ephemeral in-process HTTPS fixtures. The harness generates a loopback port, CA, and all bind/user credentials for a single command, cleans them up afterwards, and is documented in the [LDAP protocol test harness](../how-to/ldap-protocol-test-harness.md).
- [x] ✅ Add `test:identity-contract`, `test:identity-integration`, and `test:config-bundles` scripts plus the `test:authz-refactor` root aggregate. The aggregate composes named `test:authz:identity`, `test:authz:config`, and `test:authz:runtime` lanes, making its protocol, configuration, and runtime-authorization coverage independently runnable while preserving the complete order. The identity lanes cover provider-neutral OIDC, SAML, and LDAP contracts; retired legacy-mapping fixture files are not invoked. The browser-local lifecycle is an explicit guarded `test:identity:browser` lane and runs in the Playwright CI container with E2E seeding disabled.
...
- [x] ✅ Archive a sanitized browser-lifecycle status/failure code, provider key, lifecycle event names, and mock sync-run ids as a short-retention CI artifact. The artifact writer never includes Playwright traces, error text, assertions, token-like fields, directory credentials, or raw identity attributes.
...
- [x] ✅ Change local/SSO session contracts to carry principal and tenant identity; treat any legacy platform-role field as display/migration data only. New user-principal JWTs do not emit a platform-role claim, and backward-compatible middleware normalization accepts old tokens without authorizing from their claim. The refresh endpoint applies the same principal validation and persisted session-version revocation check while accepting pre-refactor refresh tokens as unambiguous user principals. User Management list/detail and authenticated profile, local-login, onboarding, and direct-LDAP responses project their compatible admin/user display field from active canonical Platform Administrators group memberships through one dedicated read helper rather than the stored column. Local login, onboarding, and provider callbacks issue refresh sessions through the shared session service; provider-bound callbacks persist their provider lineage so deactivation can revoke their sessions. Authenticated responses include the canonical response-only `session` context with a user principal and request-derived tenant id, with matching OpenAPI schemas.

  Current partial migration: the coarse browser permission snapshot now includes its request-derived tenant id. Frontend permission guards accept it only when both that tenant and the evaluated user match the authenticated response session, and fail closed while a fresh tenant-specific snapshot loads. Backend route enforcement remains authoritative.
- [x] ✅ Stop SSO provisioning and invitations from granting elevated platform roles through `User.platformRole`; map authenticated users to internal groups and assignments. New local, invited, and provider-neutral OIDC/SAML/LDAP users receive group-backed baseline access. User Management's create, pending-create, and update commands keep the persisted compatibility value non-privileged (`user`) and add/remove only their own manual Platform Administrators membership; their responses read the effective group-backed value after the transaction. Provider provisioning returns no role field at all.
- [x] ✅ Stop project/engine/member/governance services from calling `syncLegacyRoleAssignments`; write canonical assignments at the originating command boundary. `EngineService` writes managed owner/delegate assignments and current `EngineMember` operator/deployer changes as `source = manual` canonical assignments, removing any matching historical legacy projection after a compatibility-row mutation. `ProjectMemberService`, direct Starbase creation, project creation, Git clone, and Git online-create now write/remove `source = manual` per-user project-role assignments with canonical principal/scope fields; retained `ProjectMember` rows are collaboration/display compatibility records, and current role changes also remove a matching historical `source = legacy` projection so it cannot retain access. Project and engine deletion now remove every canonical assignment at the deleted resource scope, regardless of source, preventing stale manual, SSO, API, system, or migration grants from becoming orphaned; permanent local-user deletion likewise revokes direct canonical/legacy assignment aliases, group memberships, and external identity links before deleting the account. The legacy project-role writer is now migration-only. Migration `0091` creates a durable projection marker; after canonical roles are seeded, startup performs the required local compatibility projection exactly once in the same transaction that records the marker. Later startups never derive new authorization assignments from those rows.
- [x] ✅ Remove evaluator grants from platform role, ProjectMember/ProjectMemberRole, EngineMember, owner/delegate, and explicit legacy permission tables. `PermissionService.evaluatePermission`, permission-snapshot discovery, `EngineService` access/discovery/member listing, project/engine/platform compatibility middleware, the exported `requireAdmin` middleware, every `requireAction` evaluator context, and direct engine-management, audit, platform-admin, Mission Control, and Starbase permission checks rely exclusively on canonical role assignments and explicit grants. The interface compatibility export for platform-role middleware reuses the canonical permission-backed implementation, so its legacy labels cannot drift into a second evaluator. The obsolete legacy role resolver, raw-role `roleHasPermission` helper, unused direct owner/member Git repository query service, and duplicate unused ownership authorization services have been removed; shared arrays seed canonical system-role definitions only. Permission and policy evaluator contexts do not accept raw platform, project, or engine role values, and policy audit context does not record them. User Management's compatible `admin` selection writes a distinct manual platform-administrator group membership while its create, pending-create, and update paths persist only the non-privileged compatibility default and return group-projected display data; bootstrap grants survive manual demotion. The browser sends the supported `role` request field; the deprecated `platformRole` alias remains accepted only for older callers and cannot override an explicit `role`. Governance user-search does not return the stored compatibility field. Role-only compatibility middleware configuration is fail-closed, new JWTs do not emit `platformRole`, old `JwtPayload.platformRole` values cannot grant administrator access, and audit tenancy, including unredacted-view evaluation, is derived from request scope rather than that claim. Snapshot versioning now watches only canonical authorization state.
- [x] ✅ Preserve accountable owner/delegate fields only as governance metadata and require explicit effective role assignments for access. Engine access evaluation and member-list display use effective assignments, and the `my-engines` contract exposes accountable contacts under a distinct `governance` object while retaining legacy top-level fields for compatibility. Project member routes require canonical delegate-management permission rather than trusting a legacy owner membership row; ownership-transfer discovery reads the canonical project-owner assignment and uses `Project.ownerId` only as accountable recovery metadata. Project-engine access auto-approval requires the requesting user to hold canonical project-settings and engine-edit permissions rather than matching accountable metadata. The Engine Members modal does not accept an owner field it does not display or authorize from.

  Current partial migration: the authorization-filtered `/engines-api/engines` inventory now also emits the canonical `governance.accountableOwnerId`/`governance.delegateId` object while retaining the historical top-level ids. The shared contract documents that these are accountable-contact metadata; evaluator-derived assignments and action decisions remain the only access source.
...
- [x] ✅ Add `IdentityProvider`, `ExternalIdentity`, `ExternalIdentitySnapshot`, `IdentityEntitlementMapping`, `IdentitySyncRun`, and `IdentitySyncEvent` contracts with unambiguous EnterpriseGlue tenant and external directory fields. The shared identity schemas now cover the persisted provider/link/snapshot/mapping/synchronization records; `authz.ts` retains `SsoSyncRun` and `SsoSyncEvent` as aliases to the canonical provider-neutral sync contracts for compatibility.
...
- [x] ✅ Split the former 8k+ line Access Control page into tab/domain components. `GroupsPanel`, `EngineSetsPanel`, `ProjectEngineTargetsPanel`, `MachineIdentityPanel`, and the read-only `ByPrincipalPanel`/`ByResourcePanel` pair own focused rendering; the Project Engine Target panel also carries its explicit eligibility evaluator inputs and external-API diagnostics. Parent queries and mutations remain centralized. Full typecheck plus the focused principal/resource, group, and external-registration UI tests preserve the Carbon admin navigation and behavior.
...
- [x] ✅ Retire the SSO engine-access snapshot and assignment-diagnostics panels with the discontinued legacy mapping subsystem.
- [x] ✅ Extract Effective Access, principal/resource, and machine-principal audit correlation, summaries, and audit links into `pages/access-control/auditReferences.tsx`; focused Effective Access/audit interactions retain their behavior.
...
- [x] ✅ Extract the role-assignment results table into `pages/access-control/RoleAssignmentsTable.tsx`; source/ownership badges use explicit Manual, SSO, identity-provider, config, API, and system labels. Only manual and locally overridable config assignments expose removal, while SSO-managed rows remain diagnostic-only.
- [x] ✅ Complete the Role Assignment form boundary: `RoleAssignmentForm.tsx` owns the Carbon form JSX and `RoleAssignmentsPanel.tsx` composes it with the extracted table. State transitions, eligible-role rules, static selector options, submission checks, and runtime-resource queries remain in `assignmentFormOptions.ts`.
- [x] ✅ Extract the Roles and Permissions tab bodies into `pages/access-control/RoleCatalogPanels.tsx`; the parent page retains mutations and modal state while the panels own their loading/error and focused-table presentation boundaries.
- [x] ✅ Extract the Runtime Resources tab body into `pages/access-control/RuntimeResourcesPanel.tsx`; the parent retains query, reconciliation mutation, and selected-engine state while the panel owns inventory status, controls, and presentation. Its inventory rows now import the shared `RuntimeResource` API contract rather than retaining a narrower frontend copy, and the selector's presentation-only engine option is shared by the role-assignment and inventory panels.
- [x] ✅ Consolidate Engine Members UI contracts on the shared `EngineMembersResponse`, `EngineMember`, and `PendingEngineInvite` schemas. Both the modal and engine-detail access panel import the canonical response rather than retaining divergent `grantedAt`/optional-list copies; the modal also reads scoped role summaries and canonical assignments directly, including config-owned sources. Custom effective roles render as neutral display labels rather than being treated as legacy member-role variants.
...
Engine Detail runtime-resource inventory now validates the shared sanitized persistence schema after tenant filtering, so the resource-aware authorization inventory cannot drift from its documented API contract.
...
The Platform Settings API client now also re-exports the canonical shared settings schema and engine/project authority-mode unions rather than maintaining its own copy, so the settings and engine UI compile against the same bounded values as backend validation.

The Project Members modal imports that same shared project-access authority union, avoiding a separate local interpretation of manual, transition, and SSO-managed access states.

Platform Admin governance project and engine summaries now live in the shared administrator schema and are used by both OpenAPI and the typed client; user-list rows also import the existing canonical schema instead of retaining a second transport model.

The administrator Git-provider inventory now has a shared summary contract for raw provider fields plus connection counts and boolean usage indicators. The backend serializes that contract, OpenAPI documents it, and the Platform Admin client imports it directly.

The Git-provider update validator is likewise shared across the route, OpenAPI, and typed client. OAuth secret fields remain write-only request fields and are not added to the administrator summary response.

The update route now serializes an explicit sanitized provider response rather than returning the persisted entity, preventing encrypted OAuth client settings from crossing the API boundary.

The Git-provider settings component, its enclosing Platform Settings section, and its mutation hook now use that same shared inventory and write-only update contract instead of a local provider interface, `any`, or a broad entity `Partial`. The authenticated non-admin platform-settings response is likewise a shared safe-field schema used by its backend serializer, OpenAPI declaration, and every sync-settings consumer; authority-mode and credentialless-sidecar fields remain explicit while administrative and credential settings stay absent.

Provider-neutral identity-provider writes reject secret-like fields recursively, including nested configuration objects and arrays. Only explicitly named `*Ref` values may represent credentials, matching the config export boundary and preventing a newly persisted nested value from later appearing in the administrator configuration response.

Legacy mapping migration HTTP contracts and UI controls are retired for the clean-slate rollout. Provider-neutral identity-provider writes remain governed by their own strict, redacted contracts.
...
Legacy platform-role, group, and engine-assignment mapping routes, hooks, and shared contracts are retired. Supported entitlement preview and access provisioning use the provider-neutral Identity Mappings contract below.
...
The live Process Overview bulk-operation client imports that same receipt rather than returning `unknown`; older unreferenced compatibility helpers remain isolated until their legacy routes are reconciled.
...
Historic task, variable, and user-operation collections now follow that same passthrough contract at every history endpoint, including the previously untyped variable-instance route; Process Instance Detail imports the shared historic-variable display type rather than retaining a local subset.
...
The Project Detail, Project Members, and Engine Members surfaces now import shared `RoleAssignment`, `RoleSummary`, `AuthzGroup`, `AuthzGroupMembership`, principal-type, invitation lookup/capability, direct-add/invitation mutation, deploy-grant, and manual-reissue contracts directly; project-specific aliases only name the UI context and no longer narrow or duplicate the transport schema. The lookup/capability and mutation schemas are also the OpenAPI source of truth; member-add responses explicitly distinguish a direct add from an invitation rather than incorrectly advertising a roster row with timestamps. The authorization-filtered project engine-access response now likewise has one shared schema for its backend route validation, OpenAPI, DeployDialog, Editor, Project Detail, Project Overview, and Engine Access Modal. Those frontend reads now route through one typed Starbase API client method. Engine Detail now consumes its project-target list through the existing shared target schema and typed Mission Control client. It retains evaluator-derived manual/CI eligibility and nullable tag colors while stripping accidental credential fields before the response is sent. Engine invite reissue now fails closed when required manual credentials are absent. The remaining interfaces in this unchecked item still need consolidation.

Mission Control's authorization-filtered engine inventory now also has a shared sanitized response contract. It declares credentials as write-only state (`hasCredential` with a null redaction marker), evaluator-derived `myRole` as display-only metadata, adapter capabilities, configuration provenance, lifecycle, environment, and runtime/deployment controls; the list route validates against it, OpenAPI advertises it, and the Engines page consumes the inferred shared type. Inventory-derived action helpers, registration diagnostics, filtering, detail/edit/member state, and update mutation preconditions now retain that type rather than erasing it to `any`. The unreferenced raw `/api/engines` client and its duplicated CRUD/health/member interfaces have been retired, leaving only a shared-contract client. The dashboard, selector, Git import/deployment history, project deploy targets, and runtime-resource selector now fetch that same typed sanitized inventory rather than maintaining local three-field types or `any` collections; the engine page and Git history view now fetch the shared environment-tag contract through that client too, retaining its nullable color. Platform administration now uses that same tag schema for its list/create/update API and supplies a visual fallback when a legacy tag color is absent. Its onboarding-policy reads likewise import shared contracts. Engine health and connection-test responses now share the same version and sanitized transport-diagnostic schema across OpenAPI and the health UI, and both Engine Detail health widgets fetch that one typed read client; create, update, delete, environment, and connection-test mutations now use the same typed client. Engine creation and updates consume the existing shared request types with overloads that distinguish a complete create from a partial update, and return the shared sanitized inventory response. Engine Members now obtains its roster, invitation capabilities/lookup, and project-access-request list through this client; the latter has a strict shared/OpenAPI contract and no longer claims unreturned project, requester, or role relations. The modal also uses the same client for existing member/delegate/invitation and project-access-request mutations, retaining its existing UI guards and invalidation behavior. Starbase Project Overview and Detail now reuse that client for project-to-engine access requests; its result has a shared schema used by OpenAPI and both views. The remaining interfaces in this unchecked item still need consolidation.
...
- [x] ✅ Assignment removal and snapshot invalidation: the seeded-local suite deletes the disposable scoped assignment, then proves the next permission snapshot changes version, removes the engine, and causes both collection and direct detail access to fail.
- [x] ✅ Explicit deny-overrides: normal user and API-client action middleware now consults the deny-only policy gate after the RBAC grant; focused middleware tests prove a matching deny policy wins over an otherwise valid scoped grant.
- [x] ✅ Bounded-versus-broad scope shadowing: evaluator coverage labels an engine-wide grant that shadows exact-runtime and runtime-set grants, and assignment validation warns before an operator creates redundant narrower access beneath an existing Engine or Engine Set grant.
- [x] ✅ API client and service-account parity: API-client resource-action checks and deployment eligibility are covered alongside service-account deployment eligibility, including the same denied deployment result contract for both machine principal types.
- [x] ✅ Exhaustive direct action contracts: a registry-derived suite creates one named known-action/fail-closed-resolver contract for each of the 198 registered actions. The coverage guard fails unless that suite is present and validates every registry action; this is registry-contract coverage, not a claim that every endpoint implementation is executed by one test.
- [x] ✅ Generated evaluator invariant matrix: runtime-resource visibility is evaluated against deny-all, sparse exact-resource, and allow-all decision sets. It proves every returned row was independently evaluated with the correct tenant, engine, resource kind, and exact inventory id; sibling resources can never be exposed by a grant for another row.
- [x] ✅ Endpoint route-family isolation: project and engine collection middleware runs through an actual Express request and filters each requested resource independently. Existing file/version/Git, runtime lineage, deployment, batch, and migration tests exercise the related resource and mutation families.
- [x] ✅ Expanded live runtime-resource isolation: resource-aware definition, decision, job, deployment, batch-selection, and migration tests resolve live engine lineage, reject cross-tenant or absent inventory, and preserve the engine-wide compatibility fast path only where it is explicitly configured.
- [x] ✅ Real database machine-principal parity: a local PostgreSQL integration test gives an API client and a service account the same scoped deployment role, verifies exact/sibling/cross-tenant/expired decisions, token rotation, and revocation. It also caught and fixed the missing `ServiceAccount` entity registration in all supported database adapters.
- [x] ✅ CI coverage enforcement: the PostgreSQL CI lane now runs `test:authz:structure`, which includes backend-guard enforcement, the exhaustive action-coverage guard, and lane-contract tests; integration and mutation guards remain mandatory.
- [x] ✅ Targeted authorization mutation testing in CI: `test:authz:mutation` first proves a passing baseline, then temporarily bypasses the user-action and API-client denial branches. It passes only when their focused denial tests fail, and both PostgreSQL CI workflows run the guard after integration tests.
...
- [x] ✅ No new authorization write uses a legacy user/role/member field. Current project/engine commands write canonical assignments at their command boundary; retained membership rows are collaboration/display compatibility records and cannot grant access. The migration-only local projection is durable and executes exactly once.
...
- [x] ✅ Add shared resource resolvers for process definition key, decision definition key, deployment id, and instance-inherited runtime resources. Resource-aware middleware resolves process and decision definitions from the engine before matching sanitized inventory keys, resolves jobs through their referenced process definition, resolves explicit batch instances through their inherited definition keys, and now resolves deployment ids only through active inventoried runtime resources; unknown, cross-tenant, or unauthorized resources fail closed while engine-wide grants retain their fast path.
...
- [x] ✅ Preview deployment integration and lineage schema changes, including direct-engine deployments that cannot resolve project/file origin. Direct-engine previews emit explicit warnings that discovery alone is inventory-only and that project/file lineage requires validated pipeline receipts; disabling receipts adds a stronger warning.
...
- [x] ✅ Resolve secret refs and write protected engine credential fields. Apply stores only an opaque `ref:` value in the engine credential column, verifies external-reference availability with a hash-bound preflight, and the shared runtime resolver dereferences it only when making the engine call; locally supplied credentials continue to use authenticated encryption. Config values and resolved secret bytes are omitted from apply audits and responses.
...
- [x] ✅ Add runtime resource inventory reconciliation after EnterpriseGlue deployments, engine registration/update synchronization, config-managed engine changes, pipeline receipts, and explicit admin reconciliation.
- [x] ✅ Add an explicit admin reconciliation path that discovers process and decision definitions through the configured engine client, upserts sanitized runtime inventory rows, and rematerializes Runtime Resource Sets. Deployment, receipt, and engine-sync triggers call the same inventory/reconciliation services.
- [x] ✅ Reconcile direct engine deployments by `engineId + engineDeploymentId` without duplicating proxied or receipt-backed records. Discovery keys history on `engineId + camundaDeploymentId`, updates only observed deployment metadata, and preserves stronger proxy or receipt ingestion and lineage fields; coverage exercises both source families.
- [x] ✅ Accept idempotent pipeline receipts and upgrade discovered lineage to reported lineage when references validate. The direct-engine receipt route first enforces API deployment eligibility and integration policy; receipt idempotency is tenant-scoped and conflicts fail when the key names a different project, engine, or deployment. A validated receipt upgrades a matching discovered history row to reported lineage without erasing the original ingestion source, while complete proxy lineage remains stronger.
- [x] ✅ Persist process key, decision key, version, runtime tenant id, deployment id, and source observation for every discovered artifact. Runtime reconciliation validates and writes sanitized process and decision inventory observations with the engine resource id, stable key, version, runtime tenant, deployment id, `engine_discovery` source, and observation timestamp; focused reconciliation coverage proves both artifact kinds retain those fields.
- [x] ✅ Ensure runtime resource set materialization refreshes after deployment lineage, engine label, runtime tenant, or selector changes. Pipeline receipts rematerialize after their lineage observations; engine registration and label updates now explicitly refresh their Runtime Resource Sets; and config runtime-resource-set creation/update enqueue rematerialization after selector or runtime-tenant changes.
...
- [x] ✅ Runtime authorization mode tests prove the EnterpriseGlue-authoritative default, the success gate for `mirrored_engine_backstop`, and the generated OpenAPI enum contract. `engine_native_authority` remains absent from the accepted mode schema.
