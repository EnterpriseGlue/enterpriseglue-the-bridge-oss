...
- [ ] ⬜ Complete provider-neutral conversion coverage for remaining legacy SSO mapping shapes. Exact legacy email-domain rules now convert to a sanitized `attribute` entitlement (`email_domain:<domain>`), emitted consistently by OIDC, SAML, LDAP, and test adapters; raw email addresses are not used as an entitlement. The SSO Role Mappings replacement dialog now blocks unsupported rules before mutation and states the redesign requirement. Keep unsupported dynamic, negated, custom-claim, regex, wildcard, and broad-selector mappings blocked from automatic conversion and provide a deliberate redesign path using groups, Engine Sets, and normal assignments.
