...
| `custom.*` config role | Configurable | Yes | Recommended UI behavior is read-only with a **Managed by configuration** badge, or editable with a **Configuration-linked** drift warning. |
...
- [x] ✅ UI must show a **Managed by configuration** badge for config-owned engines, roles, groups, mappings, targets, and Engine Sets.
...
- [x] ✅ Complete configuration-ownership badges across Access Control and Engine UI surfaces. Engine inventory and registration detail show configuration ownership/provenance; config-locked engines are read-only and config-warn edits are marked as drift. Access Control roles, identity mappings, groups, memberships, role assignments, Engine Sets, and Engine Set assignment usage show **Managed by configuration** or **Configuration-linked**. Config-sourced groups/memberships, custom roles, project targets, and all non-manual Engine Sets reject manual mutation at the shared service boundary.
