...
- [x] ✅ Add `GET /api/authz/legacy-mapping-coverage` as the pre-retirement diagnostic. It identifies provider-neutral replacement candidates by supported claim shape, internal group, scoped role, and exact-engine target; it labels unsupported legacy mappings as manual redesign work. Candidate status deliberately requires representative sign-in/access verification and is not an automatic evaluator-retirement decision because historical legacy rows do not retain their replacement provider identifier. Identity Mappings renders this read-only readiness table with candidate, missing, and redesign counts.
