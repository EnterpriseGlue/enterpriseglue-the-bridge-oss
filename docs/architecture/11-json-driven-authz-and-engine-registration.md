...
- [x] ✅ Generated action-route allow/deny contracts: a registry-derived suite creates a contract for every registered route, requiring a registered resolver for the allow path and a fail-closed resolver mode for the deny path; route-specific integration tests continue to cover side effects.
