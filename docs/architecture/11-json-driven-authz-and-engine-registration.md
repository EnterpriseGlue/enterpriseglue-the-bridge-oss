...
- [ ] ⬜ Preserve accountable owner/delegate fields only as governance metadata and require explicit effective role assignments for access. Engine access evaluation and member-list display use effective assignments, and the `my-engines` contract now exposes accountable contacts under a distinct `governance` object while retaining legacy top-level fields for compatibility. Project metadata and UI/API contract cleanup remain.
