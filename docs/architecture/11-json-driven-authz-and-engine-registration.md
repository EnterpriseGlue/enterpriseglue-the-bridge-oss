...
- [x] ✅ Add the first engine-access bridge from Identity Mappings: an administrator can grant an engine role to the mapping's internal group for one engine, an Engine Set, an exact runtime resource, or a Runtime Resource Set through the standard role-assignment API. In-flow managed-group creation remains part of the broader wizard milestone.
