...
- [ ] ⬜ Split the 8k+ line Access Control page into tab/domain components before adding Identity, Config Imports, and Runtime Resources. `GroupsPanel` and `EngineSetsPanel` now own their focused rendering and source-ownership guard presentation; parent queries and mutations remain centralized.
