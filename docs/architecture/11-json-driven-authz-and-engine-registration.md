...
- [ ] ⬜ Update `DeployDialog.tsx`, project deployment-target UI, deployment history, and Mission Control-Starbase bridge diagnostics for proxy/direct ingestion and lineage quality. DeployDialog and Starbase now share one project-engine access/eligibility contract, the target modal consumes the canonical project-target contract, and Mission Control deployment history can open the sanitized runtime artifact lineage for an individual record. The proxy-deployment request/result is now one shared schema used for backend validation, OpenAPI, the typed Mission Control client, and DeployDialog; it retains flat legacy request fields while browser clients use the nested options form. Starbase↔Mission Control denial feedback now preserves the backend-evaluated missing requirements and offers its tenant-safe Effective Access diagnostic link, without reproducing the composite authorization decision in the browser. Proxy/direct receipt mutation remains.
