...
- [x] ✅ Bind SAML start/callback state to an optional exact provider id. Explicit-provider login resolves the same configured SAML provider for authorization redirect and assertion validation; legacy no-provider SAML login remains compatible. Generic OIDC, Microsoft configuration migration, LDAP modes, and provider-bound metadata remain pending.
