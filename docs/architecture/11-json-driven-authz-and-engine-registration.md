...
- `metadataDiscoveryEnabled` polls or explicitly reconciles engine deployment/process/decision metadata. It should normally remain enabled for both proxy and direct deployment paths.
...
- [ ] ⬜ Replace current SSO provider base64 marking with authenticated encryption through the shared encryption service.
...
- [ ] ⬜ Add a shared `SecretResolver`/encrypted secret store used by identity providers and engine connections; replace SSO base64 marking and plaintext engine credential consumption before config apply is enabled.
...
| Engine Detail > Deployments | Show proxied/reported/discovered/inferred badges, receipt principal, last reconciliation, and lineage gaps. | Bridge actions require verified lineage; reconciliation is admin/action guarded. |
...
- [ ] ⬜ Replace provider base64 secret marking with the shared encryption service or external secret refs.
...
- [ ] ⬜ Upsert config-managed scoped role assignments.
- [ ] ⬜ Upsert config-managed provider-neutral identity entitlement mappings.
- [ ] ⬜ Upsert config-managed project-engine targets.
...
- [ ] ⬜ Add OpenAPI schemas for every config object.
