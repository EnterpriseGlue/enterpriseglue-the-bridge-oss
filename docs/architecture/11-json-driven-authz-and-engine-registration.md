...
- [ ] ⬜ Return sanitized per-row action decisions only for rows already visible to the principal. Process-instance collections now opt into `includeActionDecisions=true`; after runtime-resource filtering and redaction, they attach only generic allowed/unavailable decisions for retry, suspension, and termination. The table and selected-row bulk controls consume those decisions instead of guessing from their intentionally runtime-resource-free permission snapshot, reporting partial denials before a batch is opened. Extend the same pattern to the remaining runtime row surfaces.
