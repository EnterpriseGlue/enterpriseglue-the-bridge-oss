...
- [ ] ⬜ Remove evaluator grants from platform role, ProjectMember/ProjectMemberRole, EngineMember, owner/delegate, and explicit legacy permission tables. `PermissionService.evaluatePermission`, permission-snapshot discovery, `EngineService` access/discovery/member listing, project/engine/platform compatibility middleware, and the exported `requireAdmin` middleware now rely exclusively on canonical role assignments and explicit grants. The obsolete legacy role resolver has been removed, role-only compatibility middleware configuration is explicitly fail-closed, a legacy `JwtPayload.platformRole` cannot grant administrator access, and audit tenancy is derived from request scope rather than that claim. Legacy permission-table and session-claim retirement remain before this item can be marked complete.
