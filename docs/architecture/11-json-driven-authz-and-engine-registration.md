...
- [ ] ⬜ Stop writing `userId`, `resourceType`, `resourceId`, and `sourceMappingId` aliases in new code, then remove them after local development data migration. Generic manual and config-bundle assignment writers now omit all four aliases and tests assert their absence; SSO-mapping, bootstrap, and engine-governance writers remain to be cut over.
