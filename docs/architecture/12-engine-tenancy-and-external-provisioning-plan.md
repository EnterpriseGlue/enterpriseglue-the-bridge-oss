...
| Provisioning channels | Journeys 1–8 pass locally, including all three Journey 7 and Journey 8 channels: 8/14 journeys and 12/30 required channel executions | `provisioning-journeys.json` reports 14/14 journeys and 30/30 channel executions from one clean commit |
...
- [ ] Execute all 14 provisioning-channel journeys below through the real local
  ...
  Journeys 1–8 are implemented and passing locally, including all three
  channels of Journeys 7 and 8: 8/14 journeys and 12/30 channel executions.
  Journeys 9–14 remain open, so this gate is intentionally not checked.
...
| 4 | Complete real-service provisioning journeys | Execute all 14 journeys through the local HTTP service and persistent database for manual UI, external API, and configuration channels where supported. Unsupported combinations must return their documented stable error. Write `provisioning-journeys.json` with 14/14 journeys, 30/30 required channel executions, and zero invalid or unexpected observations. Journeys 1–8 are currently implemented (8/14 journeys and 12/30 channel executions). Implement Journeys 9–14 in order so Effective Access lineage precedes runtime denial, revocation, transition, rotation, and decommission evidence. The artifact must remain incomplete until every observation is qualified on the same clean commit. | A channel silently changes topology/ownership, a retry duplicates state, a deny leaks data, or decommissioned access can reappear. |
