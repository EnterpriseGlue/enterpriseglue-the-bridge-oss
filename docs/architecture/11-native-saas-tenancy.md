...
| `EG_TENANT_RELEASE_EFFECT_COHORT_EPOCH` | unset | Positive durable effect-admission epoch for the exact managed release; required with a release identity in managed pooled Cloud. The settlement inventory remains fail-closed while any mutating source is uncovered. |
