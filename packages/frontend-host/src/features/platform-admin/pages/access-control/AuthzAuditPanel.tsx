...
export interface AuthzAuditFilterState { userId: string; resourceType: string; resourceId: string; decision: AuthzAuditDecisionFilter; limit: number; }
export const DEFAULT_AUTHZ_AUDIT_FILTER: AuthzAuditFilterState = { userId: '', resourceType: '', resourceId: '', decision: 'all', limit: 50 };
...
export function AuthzAuditPanel({ entries, loading, filters, onFiltersChange, onClearFilters }: {
  ...
  return <TableContainer><DataTable rows={entries.map((entry) => ({
    id: entry.id, timestamp: formatTimestamp(entry.timestamp), decision: entry.decision, action: entry.action, user: entry.userId,
    resource: formatResource(entry), reason: entry.reason || '-', policy: entry.policyId || '-', context: formatContext(entry.context), network: formatNetwork(entry),
