...
export function formatRuntimeResourceObservedAt(value: number | string | null | undefined): string {
...
export function RuntimeResourcesPanel({ engines, selectedEngineId, resources, loading, error, canManage, reconcilePending, reconcileError, reconcileResult, onSelectEngine, onReconcile }: {
  ...
  return <div className="eg-runtime-resources-panel" style={{ display: 'grid', gap: 'var(--spacing-5)' }}>
    <div><h3 style={{ margin: 0 }}>Runtime resources</h3><p style={{ margin: 'var(--spacing-2) 0 0', color: 'var(--cds-text-secondary)' }}>Review the processes and decisions available for access control. This inventory contains identifiers and metadata, not engine payload data.</p></div>
    <div className="eg-runtime-resource-controls" style={{ display: 'flex', gap: 'var(--spacing-4)', alignItems: 'end', flexWrap: 'wrap' }}>
      ...
      <Button kind="secondary" size="sm" disabled={!selectedEngineId || !canManage || reconcilePending} onClick={onReconcile}>Refresh inventory</Button>
    ...
    {reconcileResult && !reconcileError && <InlineNotification kind="success" lowContrast title="Inventory refreshed" subtitle={`${reconcileResult.created + reconcileResult.updated} runtime resources refreshed, ${reconcileResult.deactivated} deactivated; ${reconcileResult.deployments.created + reconcileResult.deployments.updated} deployment records and ${reconcileResult.deployments.artifactsCreated} artifacts refreshed.`} hideCloseButton />}
    {loading ? <DataTableSkeleton headers={[{ key: 'key', header: 'Resource' }]} rowCount={6} /> : error ? <InlineNotification kind="error" lowContrast title="Runtime resources could not be loaded" subtitle={parseApiError(error, 'Request failed').message} hideCloseButton /> : !selectedEngine ? <InlineNotification kind="info" lowContrast title="Select an engine" subtitle="Choose an engine to inspect its runtime resource inventory." hideCloseButton /> : resources.length === 0 ? <InlineNotification kind="info" lowContrast title="No runtime resources recorded" subtitle="Connect an engine, deploy a process or decision, then refresh the inventory." hideCloseButton /> : <TableContainer className="eg-admin-data-table"><Table size="md"><TableHead><TableRow><TableHeader>Resource</TableHeader><TableHeader>Kind</TableHeader><TableHeader>Runtime tenant</TableHeader><TableHeader>Project</TableHeader><TableHeader>Source</TableHeader><TableHeader>Observed</TableHeader></TableRow></TableHead><TableBody>{resources.map((resource) => <TableRow key={resource.id}><TableCell style={{ overflowWrap: 'anywhere' }}>{resource.resourceKey}</TableCell><TableCell><Tag type={resource.resourceKind === 'process_definition' ? 'blue' : 'purple'} size="sm">{resource.resourceKind === 'process_definition' ? 'Process' : 'Decision'}</Tag></TableCell><TableCell>{resource.runtimeTenantId || '-'}</TableCell><TableCell>{resource.projectId || '-'}</TableCell><TableCell>{resource.source}</TableCell><TableCell>{formatRuntimeResourceObservedAt(resource.observedAt)}</TableCell></TableRow>)}</TableBody></Table></TableContainer>}
  </div>;
}
