...
type IdentityReplayTask = { id: string; providerId: string; status: 'queued' | 'running' | 'completed' | 'cancelled'; attempts: number; nextAttemptAt: number | null; scanned: number; created: number; removed: number; failed: number; lastError: string | null; completedAt: number | null; createdAt: number; updatedAt: number };
...
export default function ConfigurationBundleSettingsTab() {
  ...
  const loadRunDetail = async (runId: string) => {
    ...
    try {
      ...
      setSelectedRun(run); setIdentityReplayTasks(tasks);
    }
  ...
  return <Tile>
    ...
    {selectedRun && <section style={{ marginTop: 'var(--spacing-5)' }} aria-label="Configuration apply run details">
      ...
      {selectedRun.reconciliation && <p style={{ color: 'var(--cds-text-secondary)' }}>Materialized {selectedRun.reconciliation.engineSetCount} Engine Sets, {selectedRun.reconciliation.runtimeResourceSetCount} runtime resource sets, and refreshed {selectedRun.reconciliation.engineCount} engines.</p>}
      {identityReplayTasks.length > 0 && <div style={{ marginTop: 'var(--spacing-4)' }}><h5 style={{ margin: 0 }}>Stored identity replay continuation</h5>{identityReplayTasks.map((task) => <div key={task.id} style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--spacing-3)', paddingBlock: 'var(--spacing-3)', borderBottom: '1px solid var(--cds-border-subtle)' }}><strong>{task.providerId}</strong><Tag type={task.status === 'completed' ? 'green' : task.status === 'cancelled' ? 'cool-gray' : 'warm-gray'}>{task.status}</Tag><span style={{ color: 'var(--cds-text-secondary)' }}>{task.scanned} snapshots, {task.created} added, {task.removed} removed</span>{task.attempts > 0 && <Tag type="purple">Retries: {task.attempts}</Tag>}{task.nextAttemptAt && <span style={{ color: 'var(--cds-text-secondary)' }}>Next attempt {new Date(task.nextAttemptAt).toLocaleString()}</span>}{task.lastError && <span style={{ color: 'var(--cds-text-error)' }}>{task.lastError}</span>}</div>)}</div>}
