...
type FormState = {
  ...
  entityId: string; idpEntityId: string; metadataUrl: string; metadataXmlRef: string; ssoUrl: string; signingCertificateRef: string; nameIdAttribute: string; emailAttribute: string; groupAttribute: string; signatureAlgorithm: 'sha256' | 'sha512'; ldapUrl: string;
...
const emptyForm = (): FormState => ({ key: '', displayName: '', organization: '', displayOrder: '0', isPreferred: false, loginDomains: '', protocol: 'oidc', isEnabled: false, authenticationMode: 'claims_only', directoryTenantId: '', allowVerifiedEmailLinking: false, authorizationAttributeKeys: '', issuerUrl: '', clientId: '', clientSecretRef: '', callbackUrl: '', scopes: 'openid profile email', groupClaim: '', expectedAudience: '', entityId: '', idpEntityId: '', metadataUrl: '', metadataXmlRef: '', ssoUrl: '', signingCertificateRef: '', nameIdAttribute: 'nameID', emailAttribute: 'email', groupAttribute: 'groups', signatureAlgorithm: 'sha256', ldapUrl: '', ldapBindDn: '', ldapBindPasswordRef: '', ldapUserBaseDn: '', ldapUserSearchFilter: '(uid={username})', ldapUserEnumerationFilter: '(objectClass=person)', ldapPageSize: '200', ldapSubjectAttribute: 'entryUUID', ldapEmailAttribute: 'mail', ldapGroupBaseDn: '', ldapGroupIdAttribute: 'cn', ldapMembershipMode: 'memberOf', ldapNestedGroups: false, ldapTlsTrustRef: '', syncOnManual: false, syncScheduled: false, syncIntervalSeconds: '300', syncConnectorCapability: 'claim_only' });
...
export default function IdentityProvidersSettingsTab({
  ...
  return <>
    ...
    <Modal open={open} size="lg" passiveModal={providerViewOnly} modalHeading={providerViewOnly ? 'View identity provider configuration' : editing ? 'Edit identity provider' : 'Add identity provider'} primaryButtonText={editing ? 'Save provider' : 'Create provider'} secondaryButtonText="Cancel" primaryButtonDisabled={!manage.allowed || save.isPending} selectorPrimaryFocus="#identity-provider-display-name" onRequestClose={() => setOpen(false)} onRequestSubmit={submitProvider}>
      ...
      <Select id="identity-provider-mode" labelText="Sign-in use" helperText="Host-managed providers are configuration records for a separately verified host or plugin integration. EnterpriseGlue's built-in login routes do not accept gateway claims." value={form.authenticationMode} onChange={(event) => update('authenticationMode', event.target.value as AuthenticationMode)}><SelectItem value="claims_only" text="Managed by a verified host integration" /><SelectItem value="direct" text="Users sign in through this provider" /></Select>
      {form.authenticationMode === 'claims_only' && <Callout kind="warning" lowContrast title="No built-in gateway-claims login" subtitle="This provider will not appear on the sign-in page. EnterpriseGlue 0.11 does not provide an HTTP header or reverse-proxy claims-ingestion route; the host integration must verify identity before using this namespace." />}
      ...
      <Callout kind="info" lowContrast title="Sign-in stops if memberships cannot be refreshed" subtitle="This fail-closed behavior is mandatory: EnterpriseGlue does not create a session from stale provider-managed access." />
