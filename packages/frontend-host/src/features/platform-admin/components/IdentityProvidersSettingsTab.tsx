...
type FormState = {
  ...
  syncOnLogin: boolean; syncOnManual: boolean; syncScheduled: boolean; syncIntervalSeconds: string; syncRequiredForLogin: boolean; syncIncompleteEntitlements: 'fail_closed' | 'preserve_previous'; syncConnectorCapability: 'claim_only' | 'ldap_directory' | 'scim' | 'graph';
};

const emptyForm = (): FormState => ({ key: '', protocol: 'oidc', isEnabled: false, authenticationMode: 'claims_only', directoryTenantId: '', allowVerifiedEmailLinking: false, authorizationAttributeKeys: '', issuerUrl: '', clientId: '', clientSecretRef: '', callbackUrl: '', scopes: 'openid profile email', groupClaim: '', expectedAudience: '', entityId: '', metadataUrl: '', metadataXmlRef: '', ssoUrl: '', signingCertificateRef: '', nameIdAttribute: 'nameID', emailAttribute: 'email', groupAttribute: 'groups', signatureAlgorithm: 'sha256', ldapUrl: '', ldapBindDn: '', ldapBindPasswordRef: '', ldapUserBaseDn: '', ldapUserSearchFilter: '(uid={username})', ldapUserEnumerationFilter: '(objectClass=person)', ldapPageSize: '200', ldapSubjectAttribute: 'entryUUID', ldapEmailAttribute: 'mail', ldapGroupBaseDn: '', ldapGroupIdAttribute: 'cn', ldapMembershipMode: 'memberOf', ldapNestedGroups: false, ldapTlsTrustRef: '', syncOnLogin: true, syncOnManual: false, syncScheduled: false, syncIntervalSeconds: '300', syncRequiredForLogin: true, syncIncompleteEntitlements: 'fail_closed', syncConnectorCapability: 'claim_only' });
...
export default function IdentityProvidersSettingsTab() {
  ...
  return <>
    ...
    <Modal open={open} size="lg" modalHeading={editing ? 'Edit identity provider' : 'Add identity provider'} primaryButtonText={editing ? 'Save' : 'Add'} secondaryButtonText="Cancel" primaryButtonDisabled={!manage.allowed || save.isPending} onRequestClose={() => setOpen(false)} onRequestSubmit={() => save.mutate(form)}>
      ...
      <Toggle id="identity-provider-sync-login" labelText="Synchronize on sign-in" labelA="Disabled" labelB="Enabled" toggled={form.syncOnLogin} onToggle={(checked) => update('syncOnLogin', checked)} />
      ...
      <Toggle id="identity-provider-sync-required" labelText="Synchronization required for sign-in" labelA="No" labelB="Yes" toggled={form.syncRequiredForLogin} onToggle={(checked) => update('syncRequiredForLogin', checked)} />
