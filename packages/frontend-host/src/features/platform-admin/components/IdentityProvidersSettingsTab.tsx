...
type OidcClientAuthentication = 'client_secret_post' | 'apple_private_key_jwt';
...
type FormState = {
  ...
  issuerUrl: string; clientId: string; clientAuthentication: OidcClientAuthentication; clientSecretRef: string; callbackUrl: string; scopes: string; groupClaim: string; expectedAudience: string; requestedAcrValues: string; mfaAmrValues: string; mfaAcrValues: string; postLogoutRedirectUrl: string;
  oidcClientSecretValue: string; appleTeamId: string; appleKeyId: string; applePrivateKeyRef: string; applePrivateKeyValue: string;
...
function configuration(form: FormState): Record<string, unknown> {
  ...
  if (form.protocol === 'oidc') return { ...common, issuerUrl: form.issuerUrl.trim(), clientId: form.clientId.trim(), clientAuthentication: form.clientAuthentication, callbackUrl: form.callbackUrl.trim(), scopes: form.scopes.split(/\s+/).filter(Boolean), ...(form.clientAuthentication === 'apple_private_key_jwt' ? { appleTeamId: form.appleTeamId.trim(), appleKeyId: form.appleKeyId.trim(), applePrivateKeyRef: form.applePrivateKeyRef.trim() } : form.clientSecretRef.trim() ? { clientSecretRef: form.clientSecretRef.trim() } : {}), ...(form.groupClaim.trim() ? { groupClaim: form.groupClaim.trim() } : {}), ...(form.expectedAudience.trim() ? { expectedAudience: form.expectedAudience.trim() } : {}), ...(list(form.requestedAcrValues).length ? { requestedAcrValues: list(form.requestedAcrValues) } : {}), ...(list(form.mfaAmrValues).length ? { mfaAmrValues: list(form.mfaAmrValues) } : {}), ...(list(form.mfaAcrValues).length ? { mfaAcrValues: list(form.mfaAcrValues) } : {}), ...(form.postLogoutRedirectUrl.trim() ? { postLogoutRedirectUrl: form.postLogoutRedirectUrl.trim() } : {}) };
...
function tenantSecretWrites(form: FormState): SecretWrite[] {
  const candidates: SecretWrite[] = form.protocol === 'oidc'
    ? form.clientAuthentication === 'apple_private_key_jwt'
      ? [{ purpose: 'oidc.apple_private_key', value: form.applePrivateKeyValue, referenceField: 'applePrivateKeyRef' }]
...
const providerFieldIds: Partial<Record<keyof FormState, string>> = {
  ...
  scopes: 'identity-provider-scopes',
  appleTeamId: 'identity-provider-apple-team-id',
  appleKeyId: 'identity-provider-apple-key-id',
  applePrivateKeyRef: 'identity-provider-apple-private-key-ref',
  applePrivateKeyValue: 'identity-provider-apple-private-key-value',
...
function validateProviderForm(form: FormState, allowWriteOnlySecrets = false): ProviderFormErrors {
  ...
  if (form.protocol === 'oidc') {
    ...
    if (form.clientAuthentication === 'apple_private_key_jwt') {
      if (form.issuerUrl.trim().replace(/\/$/, '') !== 'https://appleid.apple.com') errors.issuerUrl = 'Sign in with Apple requires https://appleid.apple.com.';
      if (!/^[A-Z0-9]{10}$/.test(form.appleTeamId.trim())) errors.appleTeamId = 'Enter the 10-character Apple Team ID.';
      if (!/^[A-Z0-9]{10}$/.test(form.appleKeyId.trim())) errors.appleKeyId = 'Enter the 10-character Apple Key ID.';
      if (!form.applePrivateKeyRef.trim() && !(allowWriteOnlySecrets && form.applePrivateKeyValue)) errors.applePrivateKeyRef = 'An Apple private key reference or new .p8 key is required.';
      if (form.scopes.split(/\s+/).filter(Boolean).some((scope) => !['name', 'email'].includes(scope))) errors.scopes = 'Apple scopes may contain only name and email.';
    }
  }
...
export default function IdentityProvidersSettingsTab({
  ...
  const connectionFields: Array<keyof FormState> = form.protocol === 'oidc'
    ? ['issuerUrl', 'clientId', 'callbackUrl', 'postLogoutRedirectUrl', 'scopes', ...(form.clientAuthentication === 'apple_private_key_jwt' ? ['appleTeamId' as const, 'appleKeyId' as const, 'applePrivateKeyRef' as const] : [])]
    : form.protocol === 'saml'
  ...
  const providerConnectionItems = form.protocol === 'oidc'
    ? [
      ...
      ['Client authentication', form.clientAuthentication === 'apple_private_key_jwt' ? 'Sign in with Apple private key' : 'Client secret'],
      ...(form.clientAuthentication === 'apple_private_key_jwt' ? [
        ['Apple Team ID', configuredValue(form.appleTeamId)],
        ['Apple Key ID', configuredValue(form.appleKeyId)],
        ['Apple private key reference', configuredValue(form.applePrivateKeyRef)],
        ...
        <fieldset disabled={providerViewOnly} className="eg-settings-workflow__fieldset">
          ...
          {providerStep === 2 && <section className="eg-settings-form-column" aria-labelledby="identity-provider-step-heading">
            ...
            {form.protocol === 'oidc' && <>
              ...
              <Select id="identity-provider-oidc-client-authentication" labelText="Client authentication" value={form.clientAuthentication} onChange={(event) => {
                const clientAuthentication = event.target.value as OidcClientAuthentication;
                setForm((current) => clientAuthentication === 'apple_private_key_jwt'
                  ? { ...current, clientAuthentication, issuerUrl: 'https://appleid.apple.com', scopes: 'name email', postLogoutRedirectUrl: '', requestedAcrValues: '' }
                  : { ...current, clientAuthentication, scopes: current.scopes === 'name email' ? 'openid profile email' : current.scopes });
              }}><SelectItem value="client_secret_post" text="OIDC client secret" /><SelectItem value="apple_private_key_jwt" text="Sign in with Apple private key" /></Select>
              {form.clientAuthentication === 'apple_private_key_jwt' ? <>
                <TextInput id="identity-provider-apple-team-id" labelText="Apple Team ID" value={form.appleTeamId} {...fieldValidation('appleTeamId')} onChange={(event) => update('appleTeamId', event.target.value.toUpperCase())} helperText="The 10-character Team ID from Apple Developer membership details." />
                <TextInput id="identity-provider-apple-key-id" labelText="Apple Key ID" value={form.appleKeyId} {...fieldValidation('appleKeyId')} onChange={(event) => update('appleKeyId', event.target.value.toUpperCase())} helperText="The 10-character identifier for the Sign in with Apple key." />
                <TextInput id="identity-provider-apple-private-key-ref" labelText="Apple private key reference" value={form.applePrivateKeyRef} {...fieldValidation('applePrivateKeyRef')} onChange={(event) => update('applePrivateKeyRef', event.target.value)} helperText="Opaque reference to the .p8 key. The private key is never stored in provider configuration." />
                {tenantAdminMode && <TextArea id="identity-provider-apple-private-key-value" labelText={editing ? 'New Apple .p8 private key (optional)' : 'Apple .p8 private key'} value={form.applePrivateKeyValue} onChange={(event) => update('applePrivateKeyValue', event.target.value)} helperText="Write only. The key is sent directly to your tenant secret store and is never displayed again." />}
              </> : <>
              ...
              <TextInput id="identity-provider-scopes" labelText="Scopes" value={form.scopes} {...fieldValidation('scopes')} onChange={(event) => update('scopes', event.target.value)} />
