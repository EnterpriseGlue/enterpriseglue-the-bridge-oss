...
type MigrationReadiness = { ready: boolean; targetProviderKey: string; activeMappingCount: number; blockers: Array<'target_not_found' | 'target_not_direct_oidc' | 'target_disabled' | 'secret_reference_missing' | 'secret_reference_unavailable' | 'identity_mappings_missing'> };
...
export default function IdentityProvidersSettingsTab() {
  ...
  const checkMigrationReadiness = useMutation({ mutationFn: (key: string) => apiClient.get<MigrationReadiness>(`/api/identity/providers/migration-readiness?targetProviderKey=${encodeURIComponent(key)}`), onSuccess: (result) => { setMigrationReadiness(result); setError(null); }, onError: (value: unknown) => setError(parseApiError(value, 'Unable to check migration readiness').message) });
  ...
  return <>
    <Tile>
      ...
      {migrationReadiness && <InlineNotification kind={migrationReadiness.ready ? 'success' : 'warning'} title={`Migration readiness: ${migrationReadiness.targetProviderKey}`} subtitle={migrationReadiness.ready ? `Ready for an operator-managed legacy cutover with ${migrationReadiness.activeMappingCount} active identity mapping${migrationReadiness.activeMappingCount === 1 ? '' : 's'}.` : `Not ready: ${migrationReadiness.blockers.join(', ').split('_').join(' ')}.`} hideCloseButton style={{ marginBottom: 'var(--spacing-5)' }} />}
      ...
      {manage.allowed && legacyMigratableProviders.length > 0 && <div style={{ borderTop: '1px solid var(--cds-border-subtle)', paddingTop: 'var(--spacing-5)', marginBottom: 'var(--spacing-5)' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'end', gap: 'var(--spacing-3)' }}>
          <Select id="legacy-identity-provider" labelText="Legacy provider" value={legacyProviderId} onChange={(event) => setLegacyProviderId(event.target.value)} style={{ minWidth: '18rem' }}>
          ...
          <Button kind="danger" size="sm" disabled={!selectedLegacyProvider || !migrationReadiness?.ready} onClick={() => selectedLegacyProvider && migrationReadiness && setCutoverTarget({ legacyProvider: selectedLegacyProvider, targetProviderKey: migrationReadiness.targetProviderKey })}>Disable legacy provider</Button>
        </div>
              ...
              <TableBody>{tableRows.map((row) => {
                ...
                return <TableRow key={key} {...rowProps}>
                  ...
                  <TableCell><GuardedOverflowMenu size="sm" iconDescription="Provider actions">
                    ...
                    {provider.protocol === 'oidc' && <GuardedOverflowMenuItem decision={manage} itemText="Check migration readiness" disabled={checkMigrationReadiness.isPending} onClick={() => checkMigrationReadiness.mutate(provider.key)} />}
