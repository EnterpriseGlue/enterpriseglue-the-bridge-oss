...
export default function IdentityMappingsSettingsTab() {
  ...
  return <>
    ...
    <Tile style={{ marginTop: 'var(--spacing-5)' }}>
      ...
      {!retirementReadinessQuery.isLoading && retirementReadinessQuery.data && <><InlineNotification kind={retirementReadinessQuery.data.ready ? 'success' : 'warning'} title={retirementReadinessQuery.data.ready ? 'Legacy mapping retirement gate is ready' : 'Legacy mapping retirement remains blocked'} subtitle={retirementReadinessQuery.data.ready ? `${retirementReadinessQuery.data.verifiedReplacementCount} active legacy mapping${retirementReadinessQuery.data.activeLegacyMappingCount === 1 ? '' : 's'} have current verified replacements. No evaluator has been disabled.` : `${retirementReadinessQuery.data.blockers.length} blocker${retirementReadinessQuery.data.blockers.length === 1 ? '' : 's'} remain. Resolve them in the table below before planning evaluator retirement.`} hideCloseButton style={{ marginBottom: 'var(--spacing-4)' }} />{retirementReadinessQuery.data.ready && <Button kind="danger--tertiary" size="sm" disabled={!manage.allowed || includesGlobalPlatformRoleMappings} title={includesGlobalPlatformRoleMappings ? 'Global platform-role mappings must be retired from global platform scope.' : undefined} onClick={() => { setRetirementOpen(true); setRetirementConfirmation(''); }} style={{ marginBottom: 'var(--spacing-4)' }}>Disable verified legacy mappings</Button>}{includesGlobalPlatformRoleMappings && <InlineNotification kind="info" title="Global platform mappings require global scope" subtitle="This tenant view can verify their replacements, but cannot retire globally scoped platform-role mappings." hideCloseButton style={{ marginBottom: 'var(--spacing-4)' }} />}</>}
