...
export default function IdentityMappingsSettingsTab() {
  ...
  const platformRoleManage = useActionDecision('platform.sso.platform-role-mappings.manage', resource);
  ...
  const includesGlobalPlatformRoleMappings = legacyCoverage.some((item) => item.family === 'platform_role');
  return <>
    <Tile>
    ...
    <Tile style={{ marginTop: 'var(--spacing-5)' }}>
      ...
      {!retirementReadinessQuery.isLoading && retirementReadinessQuery.data && <><InlineNotification kind={retirementReadinessQuery.data.ready ? 'success' : 'warning'} title={retirementReadinessQuery.data.ready ? 'Legacy mapping retirement gate is ready' : 'Legacy mapping retirement remains blocked'} subtitle={retirementReadinessQuery.data.ready ? `${retirementReadinessQuery.data.verifiedReplacementCount} active legacy mapping${retirementReadinessQuery.data.activeLegacyMappingCount === 1 ? '' : 's'} have current verified replacements. No evaluator has been disabled.` : `${retirementReadinessQuery.data.blockers.length} blocker${retirementReadinessQuery.data.blockers.length === 1 ? '' : 's'} remain. Resolve them in the table below before planning evaluator retirement.`} hideCloseButton style={{ marginBottom: 'var(--spacing-4)' }} />{retirementReadinessQuery.data.ready && <Button kind="danger--tertiary" size="sm" disabled={!manage.allowed || (includesGlobalPlatformRoleMappings && !platformRoleManage.allowed)} title={includesGlobalPlatformRoleMappings && !platformRoleManage.allowed ? platformRoleManage.reason || 'Missing platform role mapping management permission.' : undefined} onClick={() => { setRetirementOpen(true); setRetirementConfirmation(''); }} style={{ marginBottom: 'var(--spacing-4)' }}>Disable verified legacy mappings</Button>}{includesGlobalPlatformRoleMappings && <InlineNotification kind="info" title="Global platform mappings will use global scope" subtitle="This operation requires both group-mapping and platform-role-mapping management permissions." hideCloseButton style={{ marginBottom: 'var(--spacing-4)' }} />}</>}
    ...
    <Modal open={retirementOpen} danger modalHeading="Disable verified legacy mappings" primaryButtonText={retireLegacyMappings.isPending ? 'Disabling...' : 'Disable mappings'} secondaryButtonText="Cancel" onRequestClose={() => setRetirementOpen(false)} onRequestSubmit={() => retireLegacyMappings.mutate(includesGlobalPlatformRoleMappings)} primaryButtonDisabled={!manage.allowed || (includesGlobalPlatformRoleMappings && !platformRoleManage.allowed) || retirementConfirmation !== (includesGlobalPlatformRoleMappings ? 'RETIRE_GLOBAL_LEGACY_MAPPINGS' : 'RETIRE_LEGACY_MAPPINGS') || retireLegacyMappings.isPending}>
    ...
    <Modal open={open} modalHeading={editing ? 'Edit identity mapping' : 'Add identity mapping'} primaryButtonText={editing ? 'Save' : creationStep < 3 ? 'Continue' : 'Create mapping'} secondaryButtonText={!editing && creationStep > 1 ? 'Back' : 'Cancel'} primaryButtonDisabled={Boolean(!manage.allowed || save.isPending || (!editing && !creationStepValid) || (editing && ((createGroupInFlow && (!groupsManage.allowed || !newGroupName.trim() || !newGroupKey.trim())) || (provisionAccessInFlow && (!groupsManage.allowed || !rolesManage.allowed || !provisionRoleId || !provisionResourceId)))))} onRequestClose={() => setOpen(false)} onSecondarySubmit={() => { if (!editing && creationStep > 1) setCreationStep((step) => step - 1); else setOpen(false); }} onRequestSubmit={() => { if (!editing && creationStep < 3) { setCreationStep((step) => step + 1); return; } save.mutate(form); }}>
      ...
      {!editing && creationStep === 3 && <InlineNotification kind="success" title="Ready to create atomically" subtitle={`The ${createGroupInFlow ? 'new' : 'selected'} EnterpriseGlue group, identity mapping${provisionAccessInFlow ? ', and scoped engine access assignment' : ''} will be submitted together.`} hideCloseButton />}
    </Modal>
