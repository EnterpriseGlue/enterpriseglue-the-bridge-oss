...
class LegacyMappingCoverageService {
  async getCoverage(tenantId?: string | null): Promise<LegacyMappingCoverageItem[]> {
    ...
    const [platformMappings, groupMappings, assignmentMappings, identityMappings, assignments] = await Promise.all([
    ...
    for (const mapping of platformMappings) {
      ...
      if (unsupported) { items.push({ id: mapping.id, family: 'platform_role', status: 'manual_redesign_required', reason: unsupported, candidateIdentityMappingIds: [] }); continue; }
    ...
    for (const mapping of groupMappings) {
      ...
      if (unsupported) { items.push({ id: mapping.id, family: 'group', status: 'manual_redesign_required', reason: unsupported, candidateIdentityMappingIds: [] }); continue; }
    ...
    for (const mapping of assignmentMappings) {
      if (mapping.targetSelectorType !== 'engine_id' || !mapping.targetEngineId) { items.push({ id: mapping.id, family: 'engine_assignment', status: 'manual_redesign_required', reason: 'Dynamic engine selectors require an explicit Engine Set and group assignment.', candidateIdentityMappingIds: [] }); continue; }
      const unsupported = unsupportedReason(mapping, 'engine_assignment');
      if (unsupported) { items.push({ id: mapping.id, family: 'engine_assignment', status: 'manual_redesign_required', reason: unsupported, candidateIdentityMappingIds: [] }); continue; }
