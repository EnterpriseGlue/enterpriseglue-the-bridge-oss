...
class IdentityProviderProvisioningService {
  async provisionOidcUser(provider: IdentityProvider, claims: OidcIdentityClaims): Promise<ProvisionedIdentityUser> {
    ...
    return this.provision(provider, { providerType: 'oidc', subjectId: claims.sub, email, emailVerified: claims.email_verified === true, displayName: typeof claims.name === 'string' ? claims.name : null, firstName: typeof claims.given_name === 'string' ? claims.given_name : null, lastName: typeof claims.family_name === 'string' ? claims.family_name : null, directoryTenantId: typeof claims.tid === 'string' ? claims.tid : provider.directoryTenantId, claims: claims as Record<string, unknown> });
  }
