...
registry.registerPath({ method: 'post', path: '/api/authz/project-engine-targets/sync-legacy', ...authzExtension('platform.project-engine-targets.manage', 'POST', '/api/authz/project-engine-targets/sync-legacy'), request: { body: { content: { 'application/json': { schema: z.object({ projectId: z.string().min(1) }) } } } }, responses: { 200: { description: 'Legacy project-engine access mirrored into targets', content: { 'application/json': { schema: ProjectEngineTargetSyncLegacyResponseSchema } } } } });
