...
registry.registerPath({ method: 'post', path: '/api/identity/provisioning-directories/{key}/credentials', ...authzExtension('platform.sso.providers.manage', 'POST', '/api/identity/provisioning-directories/{key}/credentials'), request: { params: ProvisioningDirectoryParamsSchema, body: { content: { 'application/json': { schema: IdentityProvisioningCredentialCreateSchema } } } }, responses: { 201: { description: 'Provisioning credential issued; the bearer token is returned exactly once', content: { 'application/json': { schema: IdentityProvisioningCredentialIssuedSchema } } } } });
registry.registerPath({ method: 'post', path: '/api/identity/provisioning-directories/{key}/credentials/{credentialId}/rotate', ...authzExtension('platform.sso.providers.manage', 'POST', '/api/identity/provisioning-directories/{key}/credentials/{credentialId}/rotate'), request: { params: ProvisioningCredentialParamsSchema, body: { content: { 'application/json': { schema: IdentityProvisioningCredentialRotateSchema } } } }, responses: { 201: { description: 'Replacement credential issued once with bounded overlap for the old credential', content: { 'application/json': { schema: IdentityProvisioningCredentialIssuedSchema } } } } });
