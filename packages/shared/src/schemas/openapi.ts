...
const LegacyIdentityProviderCutoverResponseSchema = z.object({
  legacyProvider: z.object({ id: z.string(), name: z.string(), type: z.enum(['microsoft', 'google', 'oidc']) }),
...
registry.registerPath({ method: 'post', path: '/api/authz/external-engines/{id}/decommission', ...authzExtension('platform.external-engines.lifecycle.manage', 'POST', '/api/authz/external-engines/{id}/decommission'), request: { params: z.object({ id: z.string() }), body: { content: { 'application/json': { schema: z.object({ reason: z.string().optional() }).partial() } } } }, responses: { 200: { description: 'Externally registered engine decommissioned by platform admin', content: { 'application/json': { schema: z.object({ decommissioned: z.boolean(), engineId: z.string(), externalId: z.string().nullable(), lifecycleStatus: z.literal('decommissioned') }) } } } } });
registry.registerPath({ method: 'post', path: '/api/authz/external-engines/{id}/reactivate', ...authzExtension('platform.external-engines.lifecycle.manage', 'POST', '/api/authz/external-engines/{id}/reactivate'), request: { params: z.object({ id: z.string() }), body: { content: { 'application/json': { schema: z.object({ reason: z.string().optional() }).partial() } } } }, responses: { 200: { description: 'Externally registered engine reactivated by platform admin', content: { 'application/json': { schema: z.object({ reactivated: z.boolean(), engineId: z.string(), externalId: z.string().nullable(), lifecycleStatus: z.literal('active'), driftStatus: z.string(), materializationResults: z.array(z.record(z.string(), z.unknown())), materializationDiagnostics: ExternalEngineMaterializationDiagnosticsSchema }) } } } } });
registry.registerPath({ method: 'post', path: '/api/authz/external-engines/{id}/reconcile', ...authzExtension('platform.external-engines.reconcile', 'POST', '/api/authz/external-engines/{id}/reconcile'), request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Reconcile external engine capability and Engine Set materialization state', content: { 'application/json': { schema: z.object({ engineId: z.string(), externalId: z.string().nullable(), lifecycleStatus: EngineLifecycleStatusSchema, capabilityStatus: EngineCapabilityStatusSchema, capabilityDiagnostics: ExternalEngineCapabilityDiagnosticsSchema, materializationResults: z.array(z.record(z.string(), z.unknown())), materializationDiagnostics: ExternalEngineMaterializationDiagnosticsSchema }) } } } } });
...
registry.registerPath({ method: 'get', path: '/api/authz/runtime-resources', ...authzExtension('platform.engine-sets.read', 'GET', '/api/authz/runtime-resources'), request: { query: z.object({ engineId: z.string(), resourceKind: z.enum(['process_definition', 'decision_definition']).optional(), includeInactive: z.enum(['true', 'false']).optional() }) }, responses: { 200: { description: 'List persisted runtime resource inventory for an engine', content: { 'application/json': { schema: z.array(z.object({ id: z.string(), engineId: z.string(), resourceKind: z.string(), resourceKey: z.string(), runtimeTenantId: z.string(), isActive: z.boolean() }).passthrough()) } } } } });
registry.registerPath({ method: 'get', path: '/api/authz/runtime-resource-sets', ...authzExtension('platform.engine-sets.read', 'GET', '/api/authz/runtime-resource-sets'), request: { query: z.object({ engineId: z.string().optional(), includeArchived: z.enum(['true', 'false']).optional() }) }, responses: { 200: { description: 'List Runtime Resource Sets', content: { 'application/json': { schema: z.array(z.object({ id: z.string(), key: z.string(), engineId: z.string(), resourceKind: z.string(), selectorJson: z.string(), isArchived: z.boolean() }).passthrough()) } } } } });
registry.registerPath({ method: 'post', path: '/api/authz/runtime-resource-sets/{id}/materialize', ...authzExtension('platform.engine-sets.manage', 'POST', '/api/authz/runtime-resource-sets/:id/materialize'), request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Refresh Runtime Resource Set materialization', content: { 'application/json': { schema: z.object({ runtimeResourceSetId: z.string(), matched: z.number(), created: z.number(), updated: z.number(), removed: z.number() }) } } } } });
registry.registerPath({ method: 'post', path: '/api/authz/runtime-resources/{id}/reconcile', ...authzExtension('platform.engine-sets.manage', 'POST', '/api/authz/runtime-resources/:id/reconcile'), request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Discover runtime definitions and deployments, deactivate absent resources, and refresh Runtime Resource Set materializations', content: { 'application/json': { schema: z.object({ created: z.number(), updated: z.number(), deactivated: z.number(), materializedSets: z.number(), deployments: z.object({ created: z.number(), updated: z.number(), artifactsCreated: z.number() }) }) } } } } });
...
registry.registerPath({ method: 'post', path: '/api/authz/sso-group-mappings/{id}/migrate-provider-neutral', ...authzExtension('platform.sso.group-mappings.manage', 'POST', '/api/authz/sso-group-mappings/{id}/migrate-provider-neutral'), request: { params: z.object({ id: z.string() }), body: { content: { 'application/json': { schema: z.object({ providerKey: z.string().min(1).max(128) }) } } } }, responses: { 200: { description: 'Existing equivalent provider-neutral mapping', content: { 'application/json': { schema: z.object({ legacyMappingId: z.string(), providerKey: z.string(), created: z.boolean(), identityMapping: IdentityMappingResponseSchema }) } } }, 201: { description: 'Provider-neutral mapping created while retaining the legacy mapping', content: { 'application/json': { schema: z.object({ legacyMappingId: z.string(), providerKey: z.string(), created: z.boolean(), identityMapping: IdentityMappingResponseSchema }) } } }, 400: { description: 'Legacy mapping cannot be represented safely' }, 404: { description: 'Legacy mapping, provider, or target group not found' } } });
