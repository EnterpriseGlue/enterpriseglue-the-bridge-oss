...
registry.registerPath({ method: 'post', path: '/api/identity/mappings/provision-access', ...authzExtension('platform.sso.group-mappings.manage', 'POST', '/api/identity/mappings/provision-access'), request: { body: { content: { 'application/json': { schema: IdentityMappingRequestSchema.omit({ targetGroupKey: true }).extend({ targetGroupKey: z.string().min(1).max(160).optional(), newGroup: z.object({ key: z.string().min(1).max(255), name: z.string().min(1).max(255), description: z.string().max(2000).nullable().optional() }).optional(), roleId: z.string(), resourceType: z.enum(['engine', 'engine_set', 'engine_runtime_resource', 'engine_runtime_resource_set']), resourceId: z.string() }).refine((value) => Boolean(value.targetGroupKey) !== Boolean(value.newGroup), { message: 'Provide exactly one of targetGroupKey or newGroup' }) } } } }, responses: { 201: { description: 'Identity mapping, optional new group, and scoped group access provisioned atomically', content: { 'application/json': { schema: z.object({ mapping: IdentityMappingResponseSchema, assignment: z.object({ id: z.string(), warnings: z.array(z.string()) }), createdGroup: z.object({ id: z.string() }).nullable() }) } } } } });
