...
registry.registerPath({ method: 'get', path: '/api/authz/project-engine-targets', ...authzExtension('platform.project-engine-targets.read', 'GET', '/api/authz/project-engine-targets'), request: { query: z.object({ projectId: z.string().optional(), engineId: z.string().optional(), status: z.enum(['active', 'disabled', 'archived', 'all']).optional(), source: z.enum(['manual', 'legacy', 'ci', 'api', 'import', 'deployment_history', 'external', 'system', 'automation']).optional() }) }, responses: { 200: { description: 'List project-engine targets', content: { 'application/json': { schema: z.array(ProjectEngineTargetSchema) } } } } });
...
registry.registerPath({ method: 'get', path: '/starbase-api/projects/{projectId}/deployment-targets', ...authzExtension('project.deployment-targets.read', 'GET', '/starbase-api/projects/{projectId}/deployment-targets'), request: { params: z.object({ projectId: z.string() }), query: z.object({ status: z.enum(['active', 'disabled', 'archived', 'all']).optional(), source: z.enum(['manual', 'legacy', 'ci', 'api', 'import', 'deployment_history', 'external', 'system', 'automation']).optional() }) }, responses: { 200: { description: 'List deployment targets for one project', content: { 'application/json': { schema: z.array(ProjectEngineTargetSchema) } } } } });
