...
registry.registerPath({ method: 'post', path: '/api/identity/mappings/provision-access', ...authzExtension('platform.sso.group-mappings.manage', 'POST', '/api/identity/mappings/provision-access'), request: { body: { content: { 'application/json': { schema: IdentityMappingRequestSchema.extend({ roleId: z.string(), resourceType: z.enum(['engine', 'engine_set', 'engine_runtime_resource', 'engine_runtime_resource_set']), resourceId: z.string() }) } } } }, responses: { 201: { description: 'Identity mapping and scoped group access provisioned atomically', content: { 'application/json': { schema: z.object({ mapping: IdentityMappingResponseSchema, assignment: z.object({ id: z.string(), warnings: z.array(z.string()) }) }) } } } } });
