...
registry.registerPath({ method: 'get', path: '/api/auth/providers/enabled', responses: { 200: { description: 'Enabled provider-neutral direct-login options', content: { 'application/json': { schema: z.array(z.object({ id: z.string(), key: z.string(), protocol: z.enum(['oidc', 'ldap']), loginMethod: z.enum(['redirect', 'password']) })) } } } } });
registry.registerPath({ method: 'get', path: '/api/auth/providers/{providerId}/start', request: { params: z.object({ providerId: z.string() }) }, responses: { 302: { description: 'Redirect to the selected provider-neutral OIDC identity provider' }, 404: { description: 'Identity provider not found' } } });
