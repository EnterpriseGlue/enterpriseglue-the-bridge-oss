...
registry.registerPath({ method: 'post', path: '/api/authz/runtime-resources/{id}/reconcile', ...authzExtension('platform.engine-sets.manage', 'POST', '/api/authz/runtime-resources/:id/reconcile'), request: { params: z.object({ id: z.string() }) }, responses: { 200: { description: 'Discover runtime definitions, deactivate absent resources, and refresh Runtime Resource Set materializations', content: { 'application/json': { schema: z.object({ created: z.number(), updated: z.number(), deactivated: z.number(), materializedSets: z.number() }) } } } } });
