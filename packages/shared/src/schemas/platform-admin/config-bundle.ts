...
export const ConfigIdentityProviderSchema = z.discriminatedUnion('type', [
  ...
  CommonIdentityProviderSchema.extend({
    ...
    saml: z.object({
      ...
      signingCertificateRef: SecretReferenceSchema.optional(),
    }).strict().refine((value) => Boolean(value.metadataUrl || value.metadataXmlRef), 'SAML requires metadataUrl or metadataXmlRef'),
  }).strict(),
  CommonIdentityProviderSchema.extend({
    type: z.literal('ldap'),
