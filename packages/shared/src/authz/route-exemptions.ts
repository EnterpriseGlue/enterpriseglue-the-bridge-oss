...
export const AUTHZ_ROUTE_EXEMPTIONS: AuthzRouteExemption[] = [
  ...
  tokenAuthenticatedRoute('POST', '/api/t/:tenantSlug/auth/complete-onboarding', 'high', 'A one-time onboarding token authorizes completion before the normal user session is issued.'),
  tokenAuthenticatedRoute('GET', '/api/t/:tenantSlug/auth/onboarding/login-methods', 'low', 'Verified onboarding context restricts enrollment method discovery to its tenant.'),
  tokenAuthenticatedRoute('GET', '/api/t/:tenantSlug/auth/onboarding/providers/:providerId/start', 'high', 'Verified fresh onboarding context binds invitation and tenant to signed provider state; completion consumes the invitation transactionally.'),
  tokenAuthenticatedRoute('POST', '/api/t/:tenantSlug/auth/onboarding/providers/:providerId/login', 'high', 'Verified onboarding context and directory credentials initialize only the exact unused invited account in one transaction.'),
  {
    method: 'GET',
    route: '/api/t/:tenantSlug/invitations/:token',
    kind: 'public',
    risk: 'medium',
    owner: 'invitations',
  ...
  {
    method: 'POST',
    route: '/api/t/:tenantSlug/invitations/:token/verify-otp',
    kind: 'public',
    risk: 'high',
    owner: 'invitations',
  ...
  {
    method: 'POST',
    route: '/api/t/:tenantSlug/invitations/:token/redeem',
    kind: 'public',
    risk: 'high',
    owner: 'invitations',
  ...
  tokenAuthenticatedRoute('GET', '/api/auth/onboarding/login-methods', 'low', 'Verified onboarding context restricts enrollment method discovery to its tenant.'),
  tokenAuthenticatedRoute('GET', '/api/auth/onboarding/providers/:providerId/start', 'high', 'Verified fresh onboarding context binds invitation and tenant to signed provider state; completion consumes the invitation transactionally.'),
  tokenAuthenticatedRoute('POST', '/api/auth/onboarding/providers/:providerId/login', 'high', 'Verified onboarding context and directory credentials initialize only the exact unused invited account in one transaction.'),
  ...
  tokenAuthenticatedRoute('PUT', '/api/workloads/releases/:releaseId/effect-cohorts/:cohortEpoch', 'critical', 'The private release-controller credential opens only the release and cohort epoch pinned to this runtime, with inventory and revision binding.'),
  tokenAuthenticatedRoute('POST', '/api/workloads/releases/:releaseId/effect-cohorts/:cohortEpoch/close', 'critical', 'The private release-controller credential closes durable effect admission through a conditional revision fence.'),
  tokenAuthenticatedRoute('GET', '/api/workloads/releases/:releaseId/effect-cohorts/:cohortEpoch', 'high', 'The private release-controller credential reads sanitized source counts and fail-closed coverage for this runtime cohort.'),
  tokenAuthenticatedRoute('POST', '/api/workloads/releases/:releaseId/effect-cohorts/:cohortEpoch/verify', 'critical', 'The private release-controller credential can mark a cohort settled only when the durable inventory, assignments, and every authoritative source prove eligibility.'),
  ...
  publicRoute('POST', '/api/auth/identity/callback', 'high', 'platform-auth', 'Consumes a signed, browser-bound OIDC form-post callback before issuing a local session.'),
  ...
  publicRoute('POST', '/api/auth/cloud-signup/email/request', 'high', 'platform-auth', 'Rate-limited, non-enumerating email proof initiation creates no user or tenant membership.'),
  publicRoute('GET', '/api/auth/cloud-signup/email/verify', 'high', 'platform-auth', 'An expiring opaque email token establishes browser proof without creating a user or session.'),
  publicRoute('POST', '/api/auth/cloud-signup/email/passkey/options', 'high', 'platform-auth', 'A browser-held email proof allows a short-lived passkey registration challenge.'),
  publicRoute('POST', '/api/auth/cloud-signup/email/passkey/complete', 'critical', 'platform-auth', 'A verified email proof and user-verified WebAuthn attestation atomically create a Cloud account without membership.'),
  publicRoute('POST', '/api/auth/cloud-passkey/options', 'high', 'platform-auth', 'Creates a browser-bound, single-use passkey sign-in challenge without account enumeration.'),
  publicRoute('POST', '/api/auth/cloud-passkey/complete', 'critical', 'platform-auth', 'A user-verified WebAuthn assertion consumes its challenge before issuing a Cloud account session.'),
  ...
  publicRoute('POST', '/api/t/:tenantSlug/auth/identity/callback', 'high', 'platform-auth', 'Consumes a signed, browser-bound OIDC form-post callback after release-aware tenant routing resolves the tenant assignment.'),
  ...
  tokenAuthenticatedRoute(
    ...
    'low',
    'Authenticated users may list only sanitized tenant providers and explicitly enabled global Cloud OIDC providers that can be linked to their own account.',
  ),
  tokenAuthenticatedRoute(
    'GET',
    '/api/auth/me/identity-providers/:providerId/link',
    'high',
    'A current non-revoked tenant session starts a signed, browser-bound tenant or global Cloud OIDC authorization flow that can link only that same account.',
  ),
  {
    method: 'GET',
