...
function verifiedRecord(
  ...
  const pluginId = options.pluginId ?? 'io.enterpriseglue.example-diagnostics';
  ...
  const resources = {
    ...
    configuration: [
      {
        ...
        reference: 'example-diagnostics-api-token',
        required: true,
      },
      {
        name: 'SIGNED_ENTITLEMENT',
        source: 'deployment_file',
        reference: 'example-diagnostics-entitlement.json',
        required: true,
      },
      {
        name: 'REQUIRED_MODE',
    ...
    network: {
      ...
      egressPolicy: 'example-diagnostics-cloud',
    },
    probes: {
  ...
  const manifest = {
    ...
    deployment: {
      backend: {
        image: `registry.example/example-diagnostics@sha256:${imageHash}`,
        healthPath: '/_plugin/health',
        readyPath: '/_plugin/ready',
...
describe('plugin installer desired state', () => {
  it('installs disabled, enables, upgrades, and rolls back reversibly', () => {
    ...
    expect(installed.plugins['io.enterpriseglue.example-diagnostics']?.enabled).toBe(
      false,
    );

    const enabled = setPluginEnabledV1(
      installed,
      'io.enterpriseglue.example-diagnostics',
      true,
    ...
    expect(upgraded.plugins['io.enterpriseglue.example-diagnostics']?.version).toBe(
      '1.1.0',
    );
    expect(upgraded.plugins['io.enterpriseglue.example-diagnostics']?.enabled).toBe(
      true,
    );
    ...
    expect(rolledBack.plugins['io.enterpriseglue.example-diagnostics']?.version).toBe(
      '1.0.0',
    );
  });
  ...
  it('plans schema-compatible migration and reversible rollback phases', () => {
    ...
    const upgradedRecord =
      upgraded.plugins['io.enterpriseglue.example-diagnostics']!;
    const previousRecord =
      upgraded.previous['io.enterpriseglue.example-diagnostics']![0]!;
    ...
    expect(
      rolledBack.plugins['io.enterpriseglue.example-diagnostics']?.dataSchemaVersion,
    ).toBe(0);
  });
  ...
  it('rejects incompatible migration starts and rollback beyond the signed boundary', () => {
    ...
    expect(
      planPluginUpgradeV1(
        installed.plugins['io.enterpriseglue.example-diagnostics']!,
        irreversible,
      ).rollbackSupported,
    ).toBe(false);
    expect(() =>
    ...
    expect(
      upgraded.plugins['io.enterpriseglue.example-diagnostics']?.dataSchemaVersion,
    ).toBe(2);
  });
  ...
  it('normalizes pre-schema-field state without weakening validation', () => {
    ...
    delete legacy.plugins['io.enterpriseglue.example-diagnostics']?.dataSchemaVersion;

    const normalized = parsePluginInstallerStateV1(legacy);
    expect(
      normalized.plugins['io.enterpriseglue.example-diagnostics']?.dataSchemaVersion,
    ).toBe(0);
  });
  ...
  it('renders no exposed port, non-root isolation, opaque secret refs, and digest images', () => {
    ...
    expect(compose).toContain(
      `registry.example/example-diagnostics@sha256:${imageHash}`,
    );
    expect(compose).toContain('user: 65532:65532');
    expect(compose).toContain('no-new-privileges:true');
    expect(compose).toContain('external: true');
    expect(compose).toContain('example-diagnostics-api-token');
    ...
    expect(compose).toContain(
      './plugin-config-files/io.enterpriseglue.example-diagnostics/example-diagnostics-entitlement.json:/etc/enterpriseglue/plugin-config/example-diagnostics-entitlement.json:ro',
    );
    ...
    expect(helm).toContain('io.enterpriseglue.example-diagnostics');
  ...
  it('rejects unsafe staged paths and resource-policy mismatch', () => {
    ...
    expect(record.pluginId).toBe('io.enterpriseglue.example-diagnostics');
