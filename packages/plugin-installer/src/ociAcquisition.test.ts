...
describe('connected OCI plugin acquisition', () => {
  ...
  it('packages immutable acquisition tools and isolates the connected wrapper', async () => {
    ...
    expect(dockerfile).toContain(
      'ghcr.io/oras-project/oras:v1.3.3@sha256:',
    );
    expect(dockerfile).toContain(
    ...
    expect(containerNotices).toContain('| Cosign | 3.1.2 | Apache-2.0 |');
    ...
    await expect(
      acquirePluginOciPackageV1({
        ...
        cosignPolicyFile: await policyFixture({
          ...
          certificateIdentity:
            'https://github.com/EnterpriseGlue/ion-support-agent/.github/workflows/plugin-release.yml@refs/heads/main',
